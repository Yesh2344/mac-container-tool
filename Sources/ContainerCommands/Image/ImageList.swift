    ...
    public struct ImageList: AsyncLoggableCommand {
        ...
        public mutating func run() async throws {
            ...
            var images = try await ClientImage.list().filter { img in
                !Utility.isInfraImage(name: img.reference, builderImage: mac-container-toolSystemConfig.build.image, initImage: mac-container-toolSystemConfig.vminit.image)
            }
            ...
            if quiet && format == .table {
                for image in images {
                    let processedReferenceString = try ClientImage.denormalizeReference(image.reference, mac-container-toolSystemConfig: mac-container-toolSystemConfig)
                    print(processedReferenceString)
                }
                return
            }

            let resources = try await Self.buildResources(images: images, mac-container-toolSystemConfig: mac-container-toolSystemConfig)
        ...
        private static func buildResources(images: [ClientImage], mac-container-toolSystemConfig: ContainerSystemConfig) async throws -> [ImageResource] {
            var resources: [ImageResource] = []
            for image in images {
                resources.append(
                    try await image.toImageResource(mac-container-toolSystemConfig: mac-container-toolSystemConfig)
                )
            }
            return resources
        }
