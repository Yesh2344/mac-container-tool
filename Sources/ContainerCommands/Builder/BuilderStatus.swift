...
extension Application {
    public struct BuilderStatus: AsyncLoggableCommand {
        public static var configuration: CommandConfiguration {
            ...
            config.abstract = "Display the builder mac-container-tool status"
            return config
        }
        ...
        public func run() async throws {
            do {
                ...
                if format == .table && quiet && mac-container-tool.status != .running {
                    return
                }

                try Output.render(
                    payload: [ManagedContainer(mac-container-tool)],
                    display: [PrintableBuilder(mac-container-tool)],
                    format: format,
                    quiet: quiet
                )
