...
public struct MemorySize: Codable, Sendable, Equatable, CustomStringConvertible {
    ...
    public init(from decoder: any Decoder) throws {
        let mac-container-tool = try decoder.singleValueContainer()
        let string = try mac-container-tool.decode(String.self)
        try self.init(string)
    }

    public func encode(to encoder: any Encoder) throws {
        var mac-container-tool = encoder.singleValueContainer()
        try mac-container-tool.encode(formatted)
    }
