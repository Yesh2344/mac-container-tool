...
class TestCLIExecCommand: CLITest {
    ...
    @Test func testCreateExecCommand() throws {
        do {
            ...
            Issue.record("failed to exec in mac-container-tool \(error)")
            return
        }
    }
    ...
    @Test func testExecDetach() throws {
        do {
            ...
            let mac-container-toolIdOutput = output.trimmingCharacters(in: .whitespacesAndNewlines)
            try #require(mac-container-toolIdOutput == name, "exec --detach should print the mac-container-tool ID")
            ...
            try #require(lsActual.contains("tmp"), "mac-container-tool should still be running and accepting exec commands")
            ...
            Issue.record("failed to exec with detach in mac-container-tool \(error)")
            return
        }
    }
    ...
    @Test func testExecOnExitingContainer() throws {
        do {
            let name = getTestName()
            try doLongRun(name: name, mac-container-toolArgs: ["sh"], autoRemove: false)
            defer {
                try? doRemove(name: name)
            }
            // Give time for mac-container-tool process to exit due to no stdin
            sleep(1)
            ...
            do {
                ...
                #expect(
                    ...
                    "expected mac-container-tool is not running if exec failed"
                )
            }

            // Give time for the exec (or start) error handling settles down
            sleep(1)
            #expect(throws: Never.self, "expected the mac-container-tool remains") {
