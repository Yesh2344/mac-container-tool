...
/// Tests that stop, kill, and delete return errors for non-existent mac-container-tools.
@Suite(.serialSuites)
class TestCLINotFound: CLITest {
    ...
    @Test func testStopNonExistentContainer() throws {
        ...
        #expect(status != 0, "stop should fail for a non-existent mac-container-tool")
    }

    @Test func testKillNonExistentContainer() throws {
        let (_, _, _, status) = try run(arguments: ["kill", "does-not-exist"])
        #expect(status != 0, "kill should fail for a non-existent mac-container-tool")
    }

    @Test func testDeleteNonExistentContainer() throws {
        let (_, _, _, status) = try run(arguments: ["delete", "does-not-exist"])
        #expect(status != 0, "delete should fail for a non-existent mac-container-tool")
    }
}
