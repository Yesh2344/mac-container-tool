...
extension Application {
    public struct SystemStop: AsyncLoggableCommand {
        ...
        public static let configuration = CommandConfiguration(
            commandName: "stop",
            abstract: "Stop all `mac-container-tool` services"
        )

        @Option(name: .shortAndLong, help: "Launchd prefix for services")
        var prefix: String = "com.apple.mac-container-tool."
        ...
        public func run() async throws {
            let log = Logger(
                label: "com.apple.mac-container-tool.cli",
                factory: { label in
            ...
            if running {
                ...
                log.info("stopping mac-container-tools", metadata: ["stopTimeoutSeconds": "\(Self.stopTimeoutSeconds)"])
                do {
                    let mac-container-tools = try await client.list().map { $0.id }
                    ...
                    log.warning("failed to stop all mac-container-tools", metadata: ["error": "\(error)"])
                }

                log.info("waiting for mac-container-tools to exit")
                do {
                    ...
                    log.warning("failed to wait for all mac-container-tools", metadata: ["error": "\(error)"])
                }
            }
