...
extension Application {
    public struct ContainerRun: AsyncLoggableCommand {
        ...
        public static let configuration = CommandConfiguration(
            commandName: "run",
            abstract: "Run a mac-container-tool")
        ...
        public func run() async throws {
            ...
            // Check if mac-container-tool with id already exists.
            let client = ContainerClient()
            let existing = try? await client.get(id: id)
            guard existing == nil else {
                throw ContainerizationError(
                    .exists,
                    message: "mac-container-tool with id \(id) already exists"
                )
            }

            let ck = try await Utility.mac-container-toolConfigFromFlags(
                id: id,
                image: image,
                arguments: arguments,
            ...
            progress.set(description: "Starting mac-container-tool")
            ...
            do {
                ...
                throw ContainerizationError(.internalError, message: "failed to run mac-container-tool: \(error)")
            }
