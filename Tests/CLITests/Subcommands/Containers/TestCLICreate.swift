...
class TestCLICreateCommand: CLITest {
    ...
    @Test func testCreateArgsPassthrough() throws {
        ...
        #expect(throws: Never.self, "expected mac-container-tool create to succeed") {
    ...
    @Test func testCreateWithMACAddress() throws {
        ...
        #expect(throws: Never.self, "expected mac-container-tool create with MAC address to succeed") {
    ...
    @Test func testPublishPortParserMaxPorts() throws {
        ...
        args.append("ghcr.io/linuxmac-container-tools/alpine:3.20")
        args.append("echo")
        args.append("\"hello world\"")

        #expect(throws: Never.self, "expected mac-container-tool create maximum port publishes to succeed") {
    ...
    @Test func testPublishPortParserTooManyPorts() throws {
        ...
        #expect(throws: CLIError.self, "expected mac-container-tool create more than maximum port publishes to fail") {
            let (_, _, error, status) = try run(arguments: args)
            defer { try? doRemove(name: name) }
            if status != 0 {
                throw CLIError.executionFailed("command failed: \(error)")
    ...
    @Test func testCreateWithFQDNName() throws {
        ...
        #expect(throws: Never.self, "expected mac-container-tool create with FQDN name to succeed") {
