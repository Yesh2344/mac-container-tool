...
public struct Utility {
    ...
    public static func mac-container-toolConfigFromFlags(
        id: String,
        image: String,
        arguments: [String],
        ...
        let initImageRef = management.initImage ?? mac-container-toolSystemConfig.vminit.image
        ...
        config.resources = try Parser.resources(
            ...
            defaultCPUs: mac-container-toolSystemConfig.mac-container-tool.cpus,
            defaultMemory: mac-container-toolSystemConfig.mac-container-tool.memory
        )
        ...
        if management.networks.contains(NetworkClient.noNetworkName) {
            ...
            config.networks = try getAttachmentConfigurations(
                mac-container-toolId: config.id,
                builtinNetworkId: builtinNetworkId,
                networks: parsedNetworks,
                dnsDomain: mac-container-toolSystemConfig.dns.domain,
            )
        ...
        if management.dnsDisabled {
            ...
            let domain = management.dns.domain ?? mac-container-toolSystemConfig.dns.domain
            config.dns = .init(
        ...
        // Parse --publish-socket arguments and add to mac-container-tool configuration
        // to enable socket forwarding from mac-container-tool to host.
    ...
    static func getAttachmentConfigurations(
        ...
        let fqdn: String?
        if !mac-container-toolId.contains(".") {
            // add default domain if it exists, and mac-container-tool ID is unqualified
            if let dnsDomain {
                fqdn = "\(mac-container-toolId).\(dnsDomain)."
            } else {
                fqdn = nil
            }
        } else {
            // use mac-container-tool ID directly if fully qualified
            fqdn = "\(mac-container-toolId)."
        }
        ...
        guard networks.isEmpty else {
            ...
            // attach the first network using the fqdn, and the rest using just the mac-container-tool ID
            return try networks.enumerated().map { item in
                ...
                guard item.offset == 0 else {
                    return AttachmentConfiguration(
                        ...
                        options: AttachmentOptions(hostname: mac-container-toolId, macAddress: macAddress, mtu: mtu)
                    )
                }
                return AttachmentConfiguration(
                    network: item.element.name,
                    options: AttachmentOptions(hostname: fqdn ?? mac-container-toolId, macAddress: macAddress, mtu: mtu)
                )
            }
        }
        ...
        return [AttachmentConfiguration(network: builtinNetworkId, options: AttachmentOptions(hostname: fqdn ?? mac-container-toolId, macAddress: nil, mtu: 1280))]
    }
