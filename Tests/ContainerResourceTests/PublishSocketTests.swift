...
///
/// `mac-container-toolPath` and `hostPath` were migrated from `URL` to `FilePath`. The
...
struct PublishSocketTests {
    ...
    func testInitAcceptsAbsolutePaths() throws {
        let socket = try PublishSocket(
            mac-container-toolPath: FilePath("/var/run/docker.sock"),
            hostPath: FilePath("/Users/me/docker.sock")
        )
        #expect(socket.mac-container-toolPath == FilePath("/var/run/docker.sock"))
    ...
    func testInitRejectsRelativeContainerPath() {
        #expect(throws: ContainerizationError.self) {
            try PublishSocket(
                mac-container-toolPath: FilePath("relative/path.sock"),
    ...
    func testEncodeProducesPlainAbsolutePath() throws {
        ...
        #expect(decoded["mac-container-toolPath"] as? String == "/var/run/docker.sock")
    ...
    func testEncodeDoesNotPercentEncode() throws {
        ...
        let socket = try PublishSocket(
            mac-container-toolPath: FilePath("/tmp/a b.sock"),
            hostPath: FilePath("/tmp/dir with spaces/sock")
        )
        let json = try JSONEncoder().encode(socket)
        let decoded = try #require(try JSONSerialization.jsonObject(with: json) as? [String: Any])
        #expect(decoded["mac-container-toolPath"] as? String == "/tmp/a b.sock")
    ...
    func testDecodePlainAbsolutePath() throws {
        let json = """
            {"mac-container-toolPath":"/var/run/docker.sock","hostPath":"/Users/me/docker.sock"}
            """.data(using: .utf8)!
    ...
    func testDecodeLegacyFileURLForm() throws {
        let json = """
            {"mac-container-toolPath":"file:///var/run/docker.sock","hostPath":"file:///Users/me/docker.sock"}
            """.data(using: .utf8)!
        let socket = try JSONDecoder().decode(PublishSocket.self, from: json)
        #expect(socket.mac-container-toolPath == FilePath("/var/run/docker.sock"))
        #expect(socket.hostPath == FilePath("/Users/me/docker.sock"))
    ...
    func testDecodeLegacyFileURLResolvesPercentEncoding() throws {
        ...
        let json = """
            {"mac-container-toolPath":"file:///tmp/a%20b.sock","hostPath":"file:///tmp/x%2Fy.sock"}
            """.data(using: .utf8)!
        let socket = try JSONDecoder().decode(PublishSocket.self, from: json)
        #expect(socket.mac-container-toolPath == FilePath("/tmp/a b.sock"))
    ...
    func testDecodeLegacyFileURLWithLocalhostHost() throws {
        let json = """
            {"mac-container-toolPath":"file://localhost/var/run/docker.sock","hostPath":"file:///host.sock"}
            """.data(using: .utf8)!
        let socket = try JSONDecoder().decode(PublishSocket.self, from: json)
        #expect(socket.mac-container-toolPath == FilePath("/var/run/docker.sock"))
    ...
    func testRoundTrip() throws {
        ...
        #expect(decoded.mac-container-toolPath == original.mac-container-toolPath)
    ...
    func testDecodeEmptyStringThrows() {
        let json = """
            {"mac-container-toolPath":"","hostPath":"/host.sock"}
            """.data(using: .utf8)!
    ...
    func testDecodeFileColonOnlyThrows() {
        ...
        let json = """
            {"mac-container-toolPath":"file:","hostPath":"/host.sock"}
            """.data(using: .utf8)!
        #expect(throws: DecodingError.self) {
            try JSONDecoder().decode(PublishSocket.self, from: json)
        }
    ...
    func testDecodeRelativePathThrows() {
        ...
        let json = """
            {"mac-container-toolPath":"relative/path.sock","hostPath":"/host.sock"}
            """.data(using: .utf8)!
        #expect(throws: DecodingError.self) {
            try JSONDecoder().decode(PublishSocket.self, from: json)
        }
    ...
    func testDecodeRelativeHostPathThrows() {
        ...
        let json = """
            {"mac-container-toolPath":"/var/run/docker.sock","hostPath":"relative/host.sock"}
            """.data(using: .utf8)!
        #expect(throws: DecodingError.self) {
            try JSONDecoder().decode(PublishSocket.self, from: json)
        }
    ...
    func testDecodeNonLocalHostFileURLThrows() {
        ...
        let json = """
            {"mac-container-toolPath":"file://example.com/etc/passwd","hostPath":"/host.sock"}
            """.data(using: .utf8)!
        #expect(throws: DecodingError.self) {
            try JSONDecoder().decode(PublishSocket.self, from: json)
        }
