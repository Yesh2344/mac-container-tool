...
extension TestCLIBuildBase {
    class CLIBuilderTest: TestCLIBuildBase {
        ...
        @Test func testBuildFromPreviousStage() throws {
            ...
            let dockerfile =
                """
                FROM ghcr.io/linuxmac-container-tools/alpine:3.20 AS layer1
        ...
        @Test func testBuildAddFromSpecialDirs() throws {
            let tempDir = URL(filePath: "/tmp/mac-container-tool/.clitests/\(testSuite)/\(testName)")
        ...
        @Test func testBuildAddAll() throws {
            ...
            let dockerfile: String =
                """
                FROM ghcr.io/linuxmac-container-tools/alpine:3.20
            ...
            #expect(outputRef.contains(imageName), "expected stdout to mac-container-tool image reference")
            #expect(try self.inspectImage(imageName) == imageName, "expected to have successfully built \(imageName)")
        }
        ...
        @Test func testBuildArg() throws {
            ...
            let dockerfile: String =
                """
                ARG TAG=unknown
                FROM ghcr.io/linuxmac-container-tools/alpine:${TAG}
                """
            try createContext(tempDir: tempDir, dockerfile: dockerfile)
        ...
        @Test func testBuildAndRun() throws {
            ...
            let mac-container-toolName = "\(name)-mac-container-tool"
            try self.build(tag: imageName, tempDir: tempDir)
            #expect(try self.inspectImage(imageName) == imageName, "expected to have successfully built \(imageName)")
            // Check if the image we built is actually in the image store, and can be used.
            try self.doLongRun(name: mac-container-toolName, image: imageName)
            defer {
                try? self.doStop(name: mac-container-toolName)
            }
            var output = try doExec(name: mac-container-toolName, cmd: ["cat", "/file"])
        ...
        @Test func testBuildAfterContextChange() throws {
            ...
            let mac-container-toolName1 = "\(name)-mac-container-tool-v1"
            ...
            // Run mac-container-tool and verify content is "initial"
            try self.doLongRun(name: mac-container-toolName1, image: imageName1)
            defer {
                try? self.doStop(name: mac-container-toolName1)
            }
            var output = try doExec(name: mac-container-toolName1, cmd: ["cat", "/bar"])
            ...
            let mac-container-toolName2 = "\(name)-mac-container-tool-v2"
            ...
            // Run mac-container-tool and verify content is "updated"
            try self.doLongRun(name: mac-container-toolName2, image: imageName2)
            defer {
                try? self.doStop(name: mac-container-toolName2)
            }
            output = try doExec(name: mac-container-toolName2, cmd: ["cat", "/bar"])
        ...
        @Test func testDockerIgnoreBasic() throws {
            ...
            let mac-container-toolName = "dockerignore-basic-\(UUID().uuidString)"
            try self.doLongRun(name: mac-container-toolName, image: imageName)
            defer { try? self.doStop(name: mac-container-toolName) }

            let includedResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/included.txt"])
            #expect(includedResult.status == 0, "included.txt should be present in the image")

            let ignoredResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/ignored.txt"])
        ...
        @Test func testDockerIgnoreDockerfileSpecific() throws {
            ...
            let mac-container-toolName = "dockerignore-specific-\(UUID().uuidString)"
            try self.doLongRun(name: mac-container-toolName, image: imageName)
            defer { try? self.doStop(name: mac-container-toolName) }

            let specificResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/specific.txt"])
            #expect(specificResult.status != 0, "specific.txt should NOT be present (ignored by Dockerfile.dockerignore)")

            let generalResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/general.txt"])
            #expect(generalResult.status == 0, "general.txt should be present (only in .dockerignore, not Dockerfile.dockerignore)")

            let listResult = try run(arguments: ["exec", mac-container-toolName, "ls", "-a"])
        ...
        @Test func testDockerIgnoreIgnoredDockerfile() async throws {
            ...
            let mac-container-toolName = "dockerignore-ignored-dockerfile"
            try self.doLongRun(name: mac-container-toolName, image: imageName)
            defer { try? self.doStop(name: mac-container-toolName) }

            let dockerfileResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/Dockerfile"])
            #expect(dockerfileResult.status != 0, "Dockerfile should NOT be present in the image")

            let dockerignoreResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/.dockerignore"])
            #expect(dockerignoreResult.status != 0, ".dockerignore should NOT be present in the image")

            let testFileResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/test.txt"])
        ...
        @Test func testDockerIgnoreSubdirDockerfile() throws {
            ...
            let mac-container-toolName = "dockerignore-subdir-\(UUID().uuidString)"
            try self.doLongRun(name: mac-container-toolName, image: imageName)
            defer { try? self.doStop(name: mac-container-toolName) }
            ...
            let secretResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/secret.txt"])
            #expect(secretResult.status != 0, "secret.txt should NOT be present (ignored by Dockerfile.dockerignore)")

            let nestedSecretResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/nested/secret.txt"])
            #expect(nestedSecretResult.status != 0, "nested/secret.txt should NOT be present (ignored by Dockerfile.dockerignore)")

            let configResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/nested/project/config.txt"])
        ...
        @Test func testDockerIgnoreCustomDockerfileName() throws {
            ...
            let mac-container-toolName = "dockerignore-custom-name-\(UUID().uuidString)"
            try self.doLongRun(name: mac-container-toolName, image: imageName)
            defer { try? self.doStop(name: mac-container-toolName) }

            let app1SpecificResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/app1-specific.txt"])
            #expect(app1SpecificResult.status != 0, "app1-specific.txt should NOT be present (ignored by app1.Dockerfile.dockerignore)")

            let genericResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/generic.txt"])
        ...
        @Test func testDockerIgnoreCustomNameSubdir() throws {
            ...
            let mac-container-toolName = "dockerignore-custom-subdir-\(UUID().uuidString)"
            try self.doLongRun(name: mac-container-toolName, image: imageName)
            defer { try? self.doStop(name: mac-container-toolName) }

            let app2IgnoreResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/from-app2-ignore.txt"])
            #expect(app2IgnoreResult.status != 0, "from-app2-ignore.txt should NOT be present (ignored by app2.Dockerfile.dockerignore)")

            let rootIgnoreResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/from-root-ignore.txt"])
            ...
            let alwaysIncludedResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/always-included.txt"])
            #expect(alwaysIncludedResult.status == 0, "always-included.txt should be present")

            let configResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/nested/project/config.yaml"])
        ...
        @Test func testDockerIgnoreCoexistingDockerfiles() throws {
            ...
            let appDockerfile =
                ...
                """
            let context: [FileSystemEntry] = [
                .file("Dockerfile", content: .data("FROM ghcr.io/linuxmac-container-tools/alpine:3.20\nWORKDIR /app\nCOPY . .\n".data(using: .utf8)!)),
            ...
            let mac-container-toolName = "dockerignore-coexisting-\(UUID().uuidString)"
            try self.doLongRun(name: mac-container-toolName, image: imageName)
            defer { try? self.doStop(name: mac-container-toolName) }

            let appSpecificResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/app-specific.txt"])
            #expect(appSpecificResult.status != 0, "app-specific.txt should NOT be present (ignored by app.Dockerfile.dockerignore)")

            let dockerfileSpecificResult = try run(arguments: ["exec", mac-container-toolName, "test", "-f", "/app/dockerfile-specific.txt"])
        ...
        @Test func testDockerIgnoreReadonlyContext() throws {
            ...
            let mac-container-toolName = "dockerignore-readonly-context-\(UUID().uuidString.prefix(6))"
            try self.doLongRun(name: mac-container-toolName, image: imageName)
            defer { try? self.doStop(name: mac-container-toolName) }
        ...
        @Test func testBuildQuotedImageDockerfileArg() throws {
            ...
            let dockerfile: String =
                """
                ARG IMAGE="ghcr.io/linuxmac-container-tools/alpine:3.20"
                FROM $IMAGE
        ...
        @Test func testBuildForwardReferencedDockerfileArg() throws {
            ...
            let dockerfile: String =
                """
                ARG ALPINE="ghcr.io/linuxmac-container-tools/alpine"
        ...
        @Test func testBuildQuotedImageBuildArg() throws {
            ...
            try self.build(
                ...
                tempDir: tempDir,
                buildArgs: [
                    "IMAGE=ghcr.io/linuxmac-container-tools/alpine:3.20"
                ]
            )
            #expect(try self.inspectImage(imageName) == imageName, "expected to have successfully built \(imageName)")
        }
        ...
        @Test func testBuildForwardReferencedBuildArg() throws {
            ...
            try self.build(
                ...
                tempDir: tempDir,
                buildArgs: [
                    "ALPINE=ghcr.io/linuxmac-container-tools/alpine"
                ]
            )
            #expect(try self.inspectImage(imageName) == imageName, "expected to have successfully built \(imageName)")
        }
