    ...
    public struct ContainerExport: AsyncLoggableCommand {
        ...
        public static var configuration: CommandConfiguration {
            CommandConfiguration(
                commandName: "export",
                abstract: "Export a mac-container-tool's filesystem as a tar archive",
            )
        }
        ...
        @Option(
            name: .shortAndLong, help: "Pathname for the saved mac-container-tool filesystem (defaults to stdout)", completion: .file(),
            transform: { str in
                URL(fileURLWithPath: str, relativeTo: .currentDirectory()).absoluteURL.path(percentEncoded: false)
            })
        var output: String?

        @Argument(help: "mac-container-tool ID")
        var id: String
