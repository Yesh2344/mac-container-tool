...
extension Application {
    ...
    struct DeleteImageImplementation {
        ...
        static func removeImage(options: RemoveImageOptions, mac-container-toolSystemConfig: ContainerSystemConfig, log: Logger) async throws {
            let (found, notFound) = try await {
                ...
                return try await ClientImage.get(names: options.images, mac-container-toolSystemConfig: mac-container-toolSystemConfig)
            }()
            ...
            for image in found {
                guard
                    !Utility.isInfraImage(
                        ...
                        builderImage: mac-container-toolSystemConfig.build.image,
                        initImage: mac-container-toolSystemConfig.vminit.image
                    )
                else {
                    continue
                }
    ...
    public struct ImageDelete: AsyncLoggableCommand {
        ...
        public mutating func run() async throws {
            ...
            try await DeleteImageImplementation.removeImage(options: options, mac-container-toolSystemConfig: mac-container-toolSystemConfig, log: log)
        }
    }
}
