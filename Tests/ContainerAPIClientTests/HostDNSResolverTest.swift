...
struct HostDNSResolverTest {
    ...
    func testHostDNSCreate() async throws {
        ...
        let resolverConfigPath = tempPath.appending(FilePath.Component("mac-container-toolization.foo.bar"))
    ...
    func testHostDNSReinitialize() async throws {
        ...
        do {
            ...
            let mac-container-toolizationError = try #require(error as? ContainerizationError)
            #expect(mac-container-toolizationError.code == .internalError)
            #expect(mac-container-toolizationError.message == "mDNSResponder restart failed with status 1")
            #expect(!isAdmin)
        }
    }
}
