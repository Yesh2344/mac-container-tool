...
public struct Bundle: Sendable {
    ...
    private static let mac-container-toolRootFsBlockFilename = "rootfs.ext4"
    private static let mac-container-toolRootFsFilename = "rootfs.json"

    static let mac-container-toolConfigFilename = "config.json"
    ...
    public var mac-container-toolRootfsBlock: URL {
        self.path.appendingPathComponent(Self.mac-container-toolRootFsBlockFilename)
    }

    private var mac-container-toolRootfsConfig: URL {
        self.path.appendingPathComponent(Self.mac-container-toolRootFsFilename)
    }

    public var mac-container-toolRootfs: Filesystem {
        get throws {
            let data = try Data(contentsOf: mac-container-toolRootfsConfig)
    ...
    public var configuration: ContainerConfiguration {
        get throws {
            try load(path: self.path.appendingPathComponent(Self.mac-container-toolConfigFilename))
        }
    }
}
...
extension Bundle {
    public static func create(
        ...
        kernel: Kernel,
        mac-container-toolConfiguration: ContainerConfiguration? = nil,
        mac-container-toolRootFilesystem: Filesystem? = nil,
        ...
        if let mac-container-toolConfiguration {
            try bundle.write(filename: Self.mac-container-toolConfigFilename, value: mac-container-toolConfiguration)
        }

        if let rootFsOverride = options?.rootFsOverride {
            try bundle.setContainerRootFs(fs: rootFsOverride)
        } else if let mac-container-toolRootFilesystem {
            let readonly = mac-container-toolConfiguration?.readOnly ?? false
            try bundle.cloneContainerRootFs(cloning: mac-container-toolRootFilesystem, readonly: readonly)
        }
...
extension Bundle {
    ...
    public func set(configuration: ContainerConfiguration) throws {
        try write(filename: Self.mac-container-toolConfigFilename, value: configuration)
    }
    ...
    public func setContainerRootFs(fs: Filesystem) throws {
        ...
        try fsData.write(to: self.mac-container-toolRootfsConfig)
    }
    ...
    public func cloneContainerRootFs(cloning fs: Filesystem, readonly: Bool = false) throws {
        ...
        let cloned = try mutableFs.clone(to: self.mac-container-toolRootfsBlock.absolutePath())
