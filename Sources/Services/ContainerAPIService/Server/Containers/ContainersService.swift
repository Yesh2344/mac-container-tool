...
public actor ContainersService {
        ...
        func getClient() throws -> RuntimeClient {
            guard let client else {
                ...
                if snapshot.status == .stopped {
                    message += ": mac-container-tool is stopped"
                }
    ...
    private static let machServicePrefix = "com.apple.mac-container-tool"
    private static let launchdDomainString = try! ServiceManager.getDomainString()

    private let log: Logger
    private let debugHelpers: Bool
    private let mac-container-toolRoot: URL
    private let pluginLoader: PluginLoader
    private let runtimePlugins: [Plugin]
    private let exitMonitor: ExitMonitor
    private let mac-container-toolSystemConfig: ContainerSystemConfig

    private let lock: AsyncLock
    private var mac-container-tools: [String: ContainerState]
    ...
    public init(
        ...
        let mac-container-toolRoot = appRoot.appendingPathComponent("mac-container-tools")
        try FileManager.default.createDirectory(at: mac-container-toolRoot, withIntermediateDirectories: true)
        self.exitMonitor = ExitMonitor(log: log)
        self.lock = AsyncLock(log: log)
        self.mac-container-toolRoot = mac-container-toolRoot
        self.pluginLoader = pluginLoader
        self.mac-container-toolSystemConfig = mac-container-toolSystemConfig
        self.log = log
        self.debugHelpers = debugHelpers
        self.runtimePlugins = pluginLoader.findPlugins().filter { $0.hasType(.runtime) }
        self.mac-container-tools = try Self.loadAtBoot(root: mac-container-toolRoot, loader: pluginLoader, log: log)
    }
        ...
        for dir in directories {
            do {
                ...
                if options?.autoRemove ?? false {
                    log.info(
                        "reap auto-remove mac-container-tool",
                        metadata: [
                            "id": "\(config.id)"
                        ])
                ...
                log.warning(
                    "failed to load mac-container-tool",
                    metadata: [
                        "path": "\(dir.path)",
                        "error": "\(error)",
                    ])
    ...
    public func list(filters: ContainerListFilters = .all) async throws -> [ContainerSnapshot] {
        ...
        return self.mac-container-tools.values.compactMap { state -> ContainerSnapshot? in
    ...
    /// Execute an operation with the current mac-container-tool list while maintaining atomicity
    /// This prevents race conditions where mac-container-tools are created during the operation
    public func withContainerList<T: Sendable>(
        ...
        try await lock.withLock(logMetadata: logMetadata) { context in
            let snapshots = await self.mac-container-tools.values.map { $0.snapshot }
            return try await operation(snapshots)
        }
    }

    /// Calculate disk usage for mac-container-tools
    ...
    public func calculateDiskUsage() async -> (Int, Int, UInt64, UInt64) {
        await lock.withLock(logMetadata: ["acquirer": "\(#function)"]) { _ in
            ...
            for (id, state) in await self.mac-container-tools {
                let bundlePath = self.mac-container-toolRoot.appendingPathComponent(id)
                let mac-container-toolSize = FileManager.default.allocatedSize(of: bundlePath)
                totalSize += mac-container-toolSize

                if state.snapshot.status == .running {
                    activeCount += 1
                } else {
                    // Stopped mac-container-tools are reclaimable
                    reclaimableSize += mac-container-toolSize
                }
            }

            return (await self.mac-container-tools.count, activeCount, totalSize, reclaimableSize)
        }
    }

    /// Get set of image references used by mac-container-tools (for disk usage calculation)
    ...
    public func getActiveImageReferences() async -> Set<String> {
        await lock.withLock(logMetadata: ["acquirer": "\(#function)"]) { _ in
            ...
            for (_, state) in await self.mac-container-tools {
    ...
    /// Create a new mac-container-tool from the provided id and configuration.
    public func create(configuration: ContainerConfiguration, kernel: Kernel, options: ContainerCreateOptions, initImage: String? = nil, runtimeData: Data? = nil) async throws {
        ...
        try await self.lock.withLock(logMetadata: ["acquirer": "\(#function)", "id": "\(configuration.id)"]) { context in
            guard await self.mac-container-tools[configuration.id] == nil else {
                throw ContainerizationError(
                    .exists,
                    message: "mac-container-tool already exists: \(configuration.id)"
                )
            }

            var allHostnames = Set<String>()
            for mac-container-tool in await self.mac-container-tools.values {
                for attachmentConfiguration in mac-container-tool.snapshot.configuration.networks {
            ...
            let path = self.mac-container-toolRoot.appendingPathComponent(configuration.id)
            ...
            do {
                ...
                let mac-container-toolImage = ClientImage(description: configuration.image)
                let imageFs = try await options.rootFsOverride == nil ? mac-container-toolImage.getCreateSnapshot(platform: configuration.platform) : nil
                ...
                self.log.debug(
                    ...
                    metadata: [
                        "id": "\(configuration.id)",
                        "kernel": "\(kernel.path)",
                        "initfs": "\(initImage ?? self.mac-container-toolSystemConfig.vminit.image)",
                    ])
                let runtimeConfig = RuntimeConfiguration(
                    path: path,
                    initialFilesystem: initFilesystem,
                    kernel: kernel,
                    mac-container-toolConfiguration: configuration,
                    mac-container-toolRootFilesystem: imageFs,
                    options: options,
    ...
    /// Bootstrap the init process of the mac-container-tool.
    public func bootstrap(id: String, stdio: [FileHandle?], dynamicEnv: [String: String]) async throws {
        ...
        try await self.lock.withLock(logMetadata: ["acquirer": "\(#function)", "id": "\(id)"]) { context in
            ...
            // We've already bootstrapped this mac-container-tool. Ideally we should be able to
            ...
            let path = self.mac-container-toolRoot.appendingPathComponent(id)
    ...
    /// Create a new process in the mac-container-tool.
    ...
    /// Start a process in a mac-container-tool. This can either be a process created via
    /// createProcess, or the init process of the mac-container-tool which requires
    /// id == processID.
        ...
        try await self.lock.withLock(logMetadata: ["acquirer": "\(#function)", "id": "\(id)", "processId": "\(processID)"]) { context in
            ...
            do {
                ...
                let waitFunc: ExitMonitor.WaitHandler = {
                    log.info("registering mac-container-tool with exit monitor")
                    let code = try await client.wait(id)
                    log.info(
                        "mac-container-tool finished in exit monitor",
                        metadata: [
                            "id": "\(id)",
                            "rc": "\(code)",
                        ])
    ...
    public func kill(id: String, processID: String, signal: String) async throws {
        ...
        // mac-container-tool's init process, follow up with the same API-server cleanup
        // that `stop` performs.
    ...
    /// Stop all mac-container-tools inside the sandbox, aborting any processes currently
    /// executing inside the mac-container-tool, before stopping the underlying sandbox.
    ...
    /// Wait waits for the mac-container-tool's init process or exec to exit and returns the
    /// exit status.
    ...
    /// Resize resizes the mac-container-tool's PTY if one exists.
    ...
    // Get the logs for the mac-container-tool.
    public func logs(id: String) async throws -> [FileHandle] {
        ...
        // Logs doesn't care if the mac-container-tool is running or not, just that
        // the bundle is there, and that the files actually exist. We do
        // first try and get the mac-container-tool state so we get a nicer error message
        // (mac-container-tool foo not found) however.
        do {
            _ = try _getContainerState(id: id)
            let path = self.mac-container-toolRoot.appendingPathComponent(id)
            let bundle = ContainerResource.Bundle(path: path)
            return [
                try FileHandle(forReadingFrom: bundle.mac-container-toolLog),
                try FileHandle(forReadingFrom: bundle.bootlog),
            ]
        } catch {
            throw ContainerizationError(
                .internalError,
                message: "failed to open mac-container-tool logs: \(error)"
            )
        }
    }
    ...
    public func copyIn(id: String, source: String, destination: String, mode: UInt32, createParents: Bool = true) async throws {
        ...
        guard state.snapshot.status == .running else {
            throw ContainerizationError(.invalidState, message: "mac-container-tool \(id) is not running")
        }
        let client = try state.getClient()
    ...
    /// Get statistics for the mac-container-tool.
    ...
    /// Delete a mac-container-tool and its resources.
    public func delete(id: String, force: Bool) async throws {
        ...
        case .running:
            if !force {
                throw ContainerizationError(
                    .invalidState,
                    message: "mac-container-tool \(id) is \(state.snapshot.status) and can not be deleted"
                )
            }
    ...
    public func mac-container-toolDiskUsage(id: String) async throws -> UInt64 {
        log.debug(
            "ContainersService: enter",
            metadata: [
                "func": "\(#function)",
        ...
        let mac-container-toolPath = self.mac-container-toolRoot.appendingPathComponent(id).path

        return FileManager.default.allocatedSize(of: URL(fileURLWithPath: mac-container-toolPath))
    }
    ...
    public func exportRootfs(id: String, archive: URL) async throws {
        ...
        guard state.snapshot.status == .stopped else {
            throw ContainerizationError(.invalidState, message: "mac-container-tool is not stopped")
        }

        let path = self.mac-container-toolRoot.appendingPathComponent(id)
        let bundle = ContainerResource.Bundle(path: path)
        let rootfs = bundle.mac-container-toolRootfsBlock
    ...
    private func handleContainerExit(id: String, code: ExitStatus?, context: AsyncLock.Context) async throws {
        if let code {
            self.log.info(
                "handling mac-container-tool exit",
                metadata: [
                    "id": "\(id)",
                    "rc": "\(code)",
                ])
    ...
    private func _cleanUp(id: String) async throws {
        ...
        // Did the exit mac-container-tool handler win?
        if self.mac-container-tools[id] == nil {
            return
        }
        ...
        do {
            ...
            self.log.warning(
                "failed to read bundle configuration during cleanup for mac-container-tool",
                metadata: [
                    "id": "\(id)",
                    "error": "\(error)",
                ])
        ...
        // possibly store the mac-container-tool ID to service label mapping
        ...
        do {
            ...
            self.log.warning(
                "failed to delete bundle for mac-container-tool",
                metadata: [
                    "id": "\(id)",
                    "error": "\(error)",
                ])
        ...
        self.mac-container-tools.removeValue(forKey: id)
    }
    ...
    private func getInitBlock(for platform: Platform, imageRef: String? = nil) async throws -> Filesystem {
        let ref = imageRef ?? mac-container-toolSystemConfig.vminit.image
        let initImage = try await ClientImage.fetch(reference: ref, platform: platform, mac-container-toolSystemConfig: mac-container-toolSystemConfig)
    ...
    private func setContainerState(_ id: String, _ state: ContainerState, context: AsyncLock.Context) async {
        self.mac-container-tools[id] = state
    }
    ...
    private func _getContainerState(id: String) throws -> ContainerState {
        let state = self.mac-container-tools[id]
        guard let state else {
            throw ContainerizationError(
                .notFound,
                message: "mac-container-tool with ID \(id) not found"
            )
        }
        return state
    }
    ...
    /// Get mac-container-tool configuration, either from existing bundle or from RuntimeConfiguration
    private static func getContainerConfiguration(at path: URL) throws -> (ContainerConfiguration, ContainerCreateOptions?) {
        ...
        do {
            ...
            // This handles mac-container-tools that were created but not started yet
            let runtimeConfig = try RuntimeConfiguration.readRuntimeConfiguration(from: path)
            guard let config = runtimeConfig.mac-container-toolConfiguration else {
                throw ContainerizationError(.internalError, message: "runtime configuration missing mac-container-tool configuration")
            }
