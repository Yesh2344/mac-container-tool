...
public actor MachinesService {
    ...
    private var `default`: MachineState? {
        ...
        // This can happen if the default mac-container-tool machine was deleted
    ...
    public func list() async throws -> [MachineSnapshot] {
        ...
        let runningIds = snapshots.compactMap { $0.status == .running ? $0.mac-container-toolId : nil }
        if !runningIds.isEmpty {
            var mac-container-tools: [ContainerSnapshot]?
            do {
                mac-container-tools = try await self.client.list(filters: ContainerListFilters(ids: runningIds))
            } catch {
                self.log.warning("failed to fetch mac-container-tool addresses: \(error)")
            }
            let addressMap = (mac-container-tools ?? []).reduce(into: [String: String]()) { result, c in
                if let addr = c.networks.first?.ipv4Address.address.description {
                    result[c.id] = addr
                }
            }
            for i in snapshots.indices where snapshots[i].status == .running {
                if let cid = snapshots[i].mac-container-toolId {
        ...
        try await self.lock.withLock { context in
            guard await self.machines[configuration.id] == nil else {
                throw ContainerizationError(
                    .exists,
                    message: "mac-container-tool machine already exists: \(configuration.id)"
                )
            }
            ...
            do {
                ...
                do {
                    ...
                    self.log.error("failed to delete bundle for mac-container-tool machine \(configuration.id)")
                }
    ...
    public func delete(id: String) async throws {
        ...
        try await self.lock.withLock { context in
            ...
            case .running:
                throw ContainerizationError(
                    .invalidState,
                    message: "mac-container-tool machine \(id) is \(state.snapshot.status)")
            default:
                break
            }
    ...
    private func _getMachineState(id: String) throws -> MachineState {
        ...
        guard let state else {
            throw ContainerizationError(
                .notFound,
                message: "mac-container-tool machine with ID \(id) not found")
        }
        return state
    }
    ...
    private nonisolated func bundlePath(id: String) throws -> FilePath {
        guard let component = FilePath.Component(id) else {
            throw ContainerizationError(
                .invalidArgument,
                message: "mac-container-tool machine ID \(id) is not a valid path component"
            )
        }
    ...
    public func boot(id: String?, dynamicEnv: [String: String] = [:]) async throws -> MachineSnapshot {
        ...
        guard let id = id ?? self.default?.id else {
            throw ContainerizationError(
                .invalidArgument,
                message: "no mac-container-tool machine specified and no default set"
            )
        }
        ...
        return try await self.lock.withLock { context in
            ...
            case .stopped:
                break
            default:
                throw ContainerizationError(.invalidState, message: "mac-container-tool machine \(id) is \(state.snapshot.status)")
            }

            let cid = "\(id)-\(UUID().uuidString.prefix(MachineConfiguration.mac-container-toolUUIDLength).lowercased())"
            guard try await self.client.list(filters: .init(ids: [cid])).isEmpty else {
                throw ContainerizationError(.internalError, message: "mac-container-tool \(cid) already exists")
            }
            ...
            do {
                ...
                state.snapshot.mac-container-toolId = cid
                state.snapshot.initialized = bundle.initialized
                await self.setMachineState(id, state, context: context)

                // Monitor mac-container-tool exit in the background so we can update mac-container-tool machine state
                // when the backing mac-container-tool stops (e.g., VM crash, kill, etc.)
                try await self.exitMonitor.track(id: id) {
                    self.log.info("registering mac-container-tool machine with exit monitor")
                    let code = try await process.wait()
                    self.log.info(
                        "mac-container-tool machine exited in exit monitor",
                ...
                state.snapshot.mac-container-toolId = nil
    ...
    public func stop(id: String) async throws {
        ...
        try await self.lock.withLock { context in
            ...
            guard let cid = state.snapshot.mac-container-toolId else {
                throw ContainerizationError(
                    .internalError,
                    message: "no mac-container-tool ID for running mac-container-tool machine"
                )
            }
    ...
    private func handleMachineExit(id: String, code: ExitStatus?, context: AsyncLock.Context) async {
        self.log.info("mac-container-tool exited for mac-container-tool machine \(id)")
    ...
    public func inspect(id: String) async throws -> MachineSnapshot {
        ...
        snapshot.diskSize = bundle.diskSize
        if snapshot.status == .running, let cid = snapshot.mac-container-toolId {
            do {
                let mac-container-tool = try await self.client.get(id: cid)
                snapshot.ipAddress = mac-container-tool.networks.first?.ipv4Address.address.description
            } catch {
                self.log.warning("failed to fetch mac-container-tool address for \(cid): \(error)")
            }
        }
        return snapshot
    }

    // Get the logs for the mac-container-tool machine
    public func logs(id: String) async throws -> [FileHandle] {
        ...
        do {
            ...
            throw ContainerizationError(
                .internalError,
                message: "failed to open mac-container-tool machine logs: \(error)")
        }
    }
}
