...
class TestCLIRunLifecycle: CLITest {
    ...
    @Test func testRunFailureCleanup() throws {
        ...
        // try to create a mac-container-tool we know will fail
        let badArgs: [String] = [
            "--rm",
            "--user",
            name,
        ]
        #expect(throws: CLIError.self, "expect mac-container-tool to fail with invalid user") {
            try self.doLongRun(name: name, args: badArgs)
        }

        // try to create a mac-container-tool with the same name but no user that should succeed
        #expect(throws: Never.self, "expected mac-container-tool run to succeed") {
    ...
    @Test func testStartIdempotent() throws {
        ...
        #expect(throws: Never.self, "expected mac-container-tool run to succeed") {
            ...
            #expect(status == 0, "expected start to succeed on already running mac-container-tool")
            #expect(output.trimmingCharacters(in: .whitespacesAndNewlines) == name, "expected output to be mac-container-tool name")

            // Don't care about the resp, just that the mac-container-tool is still there and not cleaned up.
    ...
    @Test func testStartIdempotentAttachFails() throws {
        ...
        #expect(throws: Never.self, "expected mac-container-tool run to succeed") {
            ...
            #expect(status != 0, "expected start with attach to fail on already running mac-container-tool")
            #expect(error.contains("attach is currently unsupported on already running mac-container-tools"), "expected error message about attach not supported")
    ...
    @Test func testStartPortBindFails() async throws {
        ...
        try doLongRun(
            ...
            mac-container-toolArgs: ["python3", "-m", "http.server", "\(port)"]
        )
        defer {
    ...
    @Test func testRunInvalidExcutable() async throws {
        ...
        #expect(throws: CLIError.self, "running invalid executable must throw error, not hang") {
            try doLongRun(
                name: name,
                mac-container-toolArgs: ["foobarbaz"]
            )
        }
    ...
    @Test func testSSHForwarding() throws {
        ...
        // Verify SSH_AUTH_SOCK is set to the expected guest path inside the mac-container-tool.
        ...
        #expect(
            ...
            "expected forwarded SSH socket to exist in mac-container-tool"
        )
