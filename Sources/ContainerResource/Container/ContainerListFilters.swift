...
/// Filters for listing mac-container-tools.
public struct ContainerListFilters: Sendable, Codable {
    public static func exclude(_ str: String) -> String {
        "^(?!\(str)$)"
    }

    /// Filter by mac-container-tool IDs. If non-empty, only mac-container-tools with matching IDs are returned.
    public var ids: [String]
    /// Filter by mac-container-tool status.
    ...
    /// matched against the mac-container-tool's label value. If a mac-container-tool does not have the specified key,
    /// the value is treated as an empty string. This means a positive pattern (e.g. ``^b$``) will
    /// exclude mac-container-tools without the label, while a negation pattern (e.g. ``^(?!b$)``) will
    /// include them.
    public var labels: [String: String]

    /// No filters applied. Will return all mac-container-tools.
