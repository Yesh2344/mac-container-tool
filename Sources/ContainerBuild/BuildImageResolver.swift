...
struct BuildImageResolver: BuildPipelineHandler {
    ...
    let pull: Bool
    let mac-container-toolSystemConfig: ContainerSystemConfig

    public init(_ contentStore: ContentStore, quiet: Bool = false, output: FileHandle = FileHandle.standardError, pull: Bool = false, mac-container-toolSystemConfig: ContainerSystemConfig)
        throws
    {
    ...
    func handle(_ sender: AsyncStream<ClientStream>.Continuation, _ packet: ServerStream) async throws {
        ...
        let img = try await {
            ...
            if self.pull {
                return try await ClientImage.pull(reference: ref, platform: platform, mac-container-toolSystemConfig: mac-container-toolSystemConfig, progressUpdate: progress.handler)
            }
            // Use fetch() which checks cache first, then pulls if needed
            return try await ClientImage.fetch(reference: ref, platform: platform, mac-container-toolSystemConfig: mac-container-toolSystemConfig, progressUpdate: progress.handler)
        }()
