...
public struct ContainersHarness: Sendable {
    ...
    public func list(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        let mac-container-tools = try await service.list(filters: filters)
        let data = try JSONEncoder().encode(mac-container-tools)

        let reply = message.reply()
        reply.set(key: .mac-container-tools, value: data)
        return reply
    }
    ...
    public func create(_ message: XPCMessage) async throws -> XPCMessage {
        let data = message.dataNoCopy(key: .mac-container-toolConfig)
        guard let data else {
            throw ContainerizationError(
                .invalidArgument,
                message: "mac-container-tool configuration cannot be empty"
            )
        }
        ...
        let odata = message.dataNoCopy(key: .mac-container-toolOptions)
    ...
    public func diskUsage(_ message: XPCMessage) async throws -> XPCMessage {
        guard let mac-container-toolId = message.string(key: .id) else {
            throw ContainerizationError(.invalidArgument, message: "id cannot be empty")
        }

        let size = try await service.mac-container-toolDiskUsage(id: mac-container-toolId)

        let reply = message.reply()
        reply.set(key: .mac-container-toolSize, value: size)
        return reply
    }
