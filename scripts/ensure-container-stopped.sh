...
usage() {
    echo "Usage: $0 [-a] [-h]"
    echo "Stop mac-container-tool services"
    echo
    echo "Options:"
    echo "a     Stop mac-container-tool services in all launchd domains."
...
if $ALL_DOMAINS; then
    ...
    for domain in "gui/$uid" "user/$uid" "system"; do
        ...
        launchctl print "$domain" 2>/dev/null \
            | grep -oE 'com\.apple\.mac-container-tool\.[^ ]+' \
            | sort -u \
    ...
    launchctl list | grep -e 'com\.apple\.mac-container-tool\W' | awk '{print $3}' \
