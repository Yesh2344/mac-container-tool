...
class TestCLIRmRaceCondition: CLITest {
    ...
    /// Helper method to check if a mac-container-tool exists
    private func mac-container-toolExists(_ name: String) -> Bool {
        do {
    ...
    /// Safe mac-container-tool removal that handles already-removed mac-container-tools gracefully
    private func safeRemove(name: String, force: Bool = false) throws {
        guard mac-container-toolExists(name) else {
    ...
    @Test func testStopRmRace() async throws {
        ...
        do {
            // Create and start a mac-container-tool in detached mode that runs indefinitely
            try doCreate(name: name, args: ["sleep", "infinity"])
            try doStart(name: name)

            // Wait for mac-container-tool to be running
            ...
            do {
                ...
                } else if message.contains("not found") || message.contains("failed to delete one or more mac-container-tools") {
            ...
            // If the mac-container-tool was already removed, we're done
            ...
            while removeAttempts < maxRemoveAttempts {
                do {
                    ...
                    // If mac-container-tool doesn't exist, we're done
                    if message.contains("not found") {
                        break
                    }

                    guard removeAttempts < maxRemoveAttempts - 1 else {
                        throw CLITest.CLIError.executionFailed("Failed to remove mac-container-tool after \(maxRemoveAttempts) attempts: \(message)")
                    }
            ...
            // Safe cleanup - only try to remove if mac-container-tool actually exists
