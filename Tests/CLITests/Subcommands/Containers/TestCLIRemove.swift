...
class TestCLIRemove: CLITest {
    ...
    @Test func testDeleteStopped() async throws {
        ...
        // Create without --rm so the mac-container-tool persists after being stopped
    ...
    @Test func testDeleteAllSkipsRunning() async throws {
        ...
        // Running mac-container-tool should be untouched
        #expect(try getContainerStatus(runningName) == "running")
        // Stopped mac-container-tool should be gone
    ...
    @Test func testDeleteExplicitIdsConflictWithAll() throws {
        let (_, _, error, status) = try run(arguments: ["delete", "--all", "some-mac-container-tool"])
    ...
    @Test func testDeleteDuplicateIds() async throws {
        ...
        #expect(lines.count == 1, "Expected mac-container-tool to be deleted exactly once, got \(lines.count) lines")
    }

    @Test func testInspectMissingContainerFails() throws {
        let (_, _, error, status) = try run(arguments: ["inspect", "definitely-missing-mac-container-tool"])
        #expect(status != 0, "Expected non-zero exit for missing mac-container-tool")
        #expect(error.contains("mac-container-tool not found"))
    }
}
