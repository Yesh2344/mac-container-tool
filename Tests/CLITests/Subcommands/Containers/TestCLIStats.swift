...
class TestCLIStatsCommand: CLITest {
    ...
    @Test func testStatsNoStreamJSONFormat() throws {
        ...
        #expect(throws: Never.self, "expected stats command to succeed") {
            ...
            #expect(stats.count == 1, "expected stats for one mac-container-tool")
            #expect(stats[0].id == name, "mac-container-tool ID should match")
    ...
    @Test func testStatsIdleCPUPercentage() throws {
        let name = getTestName()
        #expect(throws: Never.self, "expected stats to show low CPU for idle mac-container-tool") {
            try doLongRun(name: name, mac-container-toolArgs: ["sleep", "3600"])
            defer {
                try? doStop(name: name)
                try? doRemove(name: name)
            }
            ...
            try #require(dataLine != nil, "should find mac-container-tool data row")
            ...
            // Idle mac-container-tool should use very little CPU (less than 5%)
            #expect(cpuValue! < 5.0, "idle mac-container-tool CPU should be < 5%, got \(cpuValue!)%")
        }
    }

    @Test func testStatsHighCPUPercentage() throws {
        let name = getTestName()
        #expect(throws: Never.self, "expected stats to show high CPU for busy mac-container-tool") {
            // Run a mac-container-tool with a busy loop
            try doLongRun(name: name, mac-container-toolArgs: ["sh", "-c", "while true; do :; done"])
            defer {
                try? doStop(name: name)
                try? doRemove(name: name)
            }
            ...
            // Extract CPU percentage - it should be in the second column
            // Format is like: "mac-container-tool_id   95.23%   ..."
            let columns = dataLine!.split(separator: " ").filter { !$0.isEmpty }
            #expect(columns.count >= 2, "should have at least 2 columns")
            ...
            #expect(cpuValue! > 50.0, "busy mac-container-tool CPU should be > 50%, got \(cpuValue!)%")
    ...
    @Test func testStatsTableFormat() throws {
        ...
        #expect(throws: Never.self, "expected stats table format to work") {
            ...
            #expect(output.contains(name), "output should contain mac-container-tool name")
        }
    }
    ...
    @Test func testStatsAllContainers() throws {
        ...
        #expect(throws: Never.self, "expected stats for all mac-container-tools") {
            ...
            // Get stats for all mac-container-tools (no name specified)
            let (data, _, error, status) = try run(arguments: [
                "stats",
                "--format", "json",
                "--no-stream",
            ...
            // Should have stats for both mac-container-tools
            try #require(stats.count >= 2, "should have stats for at least 2 mac-container-tools")

            let mac-container-toolIds = stats.map { $0.id }
            #expect(mac-container-toolIds.contains(name1), "should include first mac-container-tool")
            #expect(mac-container-toolIds.contains(name2), "should include second mac-container-tool")
        }
    }

    @Test func testStatsNonExistentContainer() throws {
        #expect(throws: Never.self, "expected stats to fail for non-existent mac-container-tool") {
            let (_, _, _, status) = try run(arguments: [
                "stats",
                "--no-stream",
                "nonexistent-mac-container-tool-xyz",
            ])

            #expect(status != 0, "stats command should fail for non-existent mac-container-tool")
        }
    }
}
