...
class TestCLINoParallelCases: CLITest {
    ...
    @Test func testImagePruneDanglingImages() throws {
        ...
        let mac-container-toolName = "\(name)_mac-container-tool"
        ...
        // 3. Create a running mac-container-tool based on alpine
        try doLongRun(
            name: mac-container-toolName,
            image: alpine
        )
    ...
    @Test func testNetworkPruneSkipsNetworksInUse() throws {
        ...
        // Creation of mac-container-tool with network connection
        let port = UInt16.random(in: 50000..<60000)
        try doLongRun(
            name: mac-container-toolName,
            image: "docker.io/library/python:alpine",
            args: ["--network", networkInUse],
            mac-container-toolArgs: ["python3", "-m", "http.server", "--bind", "0.0.0.0", "\(port)"]
        )
        try waitForContainerRunning(mac-container-toolName)
        let mac-container-tool = try inspectContainer(mac-container-toolName)
        #expect(mac-container-tool.networks.count > 0)
    ...
    @Test func testNetworkPruneSkipsNetworkAttachedToStoppedContainer() async throws {
        ...
        // Prune should NOT remove the network (mac-container-tool exists, even if stopped)
        let (_, _, error, status) = try run(arguments: ["network", "prune"])
        if status != 0 {
            throw CLIError.executionFailed("network prune failed: \(error)")
        }
        ...
        let (_, listAfter, _, statusAfter) = try run(arguments: ["network", "list", "--quiet"])
        #expect(statusAfter == 0)
        #expect(listAfter.contains(networkName), "network attached to stopped mac-container-tool should NOT be pruned")
        ...
        #expect(!listFinal.contains(networkName), "network should be pruned after mac-container-tool is deleted")
    }
}
