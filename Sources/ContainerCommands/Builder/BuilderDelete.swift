...
extension Application {
    public struct BuilderDelete: AsyncLoggableCommand {
        public static var configuration: CommandConfiguration {
            ...
            config.aliases = ["rm"]
            config.abstract = "Delete the builder mac-container-tool"
            return config
        }
        ...
        public func run() async throws {
            do {
                ...
                let mac-container-tool = try await client.get(id: "buildkit")
                if mac-container-tool.status != .stopped {
                    guard force else {
                        throw ContainerizationError(.invalidState, message: "BuildKit mac-container-tool is not stopped, use --force to override")
                    }
