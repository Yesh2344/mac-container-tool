...
class TestCLIRunCommand1: CLITest {
    ...
    @Test func testRunCommand() throws {
        do {
            ...
            Issue.record("failed to run mac-container-tool \(error)")
            return
        }
    }
    ...
    @Test func testRunCommandEnv() throws {
        do {
            ...
            #expect(
                ...
                "environment variable \(envData) not set in mac-container-tool configuration")
            try doStop(name: name)
        } catch {
            Issue.record("failed to run mac-container-tool \(error)")
            return
        ...
        do {
            ...
            for item in expected {
                #expect(
                    ...
                    "environment variable \(item) not set in mac-container-tool configuration")
            }
            try doStop(name: name)
        } catch {
            Issue.record("failed to run mac-container-tool \(error)")
...
class TestCLIRunCommand2: CLITest {
    ...
    @Test func testRunCommandOSArch() throws {
        do {
            ...
            #expect(output == "\(os) \(expectedArch)", "expected mac-container-tool to use '\(os) \(expectedArch)', instead got '\(output)'")
            try doStop(name: name)
        } catch {
            Issue.record("failed to run mac-container-tool \(error)")
            return
    ...
    @Test func testRunCommandCidfile() throws {
        do {
            ...
            #expect(actualID == name, "expected mac-container-tool ID '\(name)', instead got '\(actualID)'")
            try doStop(name: name)
        } catch {
            Issue.record("failed to run mac-container-tool \(error)")
            return
    ...
    @Test func testRunCommandInit() throws {
        do {
            ...
            let inspectResp = try inspectContainer(name)
            #expect(inspectResp.configuration.useInit == true, "expected useInit to be true in mac-container-tool configuration")
            ...
            Issue.record("failed to run mac-container-tool with --init: \(error)")
            return
        }
    }
    ...
    @Test func testRunCommandWithoutInitDefault() throws {
        do {
            ...
            Issue.record("failed to run mac-container-tool without --init: \(error)")
            return
        }
    }
}
    ...
    @Test func testForwardTCP() async throws {
        ...
        do {
            ...
            try doLongRun(
                ...
                mac-container-toolArgs: ["python3", "-m", "http.server", "--bind", "0.0.0.0", "\(serverPort)"])
            defer {
                try? doStop(name: name)
            }
        ...
        for portOffset in 0..<range {
            ...
            do {
                ...
                try doLongRun(
                    ...
                    mac-container-toolArgs: ["python3", "-m", "http.server", "--bind", "0.0.0.0", "\(serverPortStart + portOffset)"])
                defer {
                    try? doStop(name: name)
                }
    ...
    @Test func testForwardTCPv6() async throws {
        ...
        do {
            ...
            try doLongRun(
                ...
                args: ["--publish", "\(proxyIp):\(proxyPort):\(serverPort)/tcp"],
                mac-container-toolArgs: ["npx", "http-server", "-a", "::", "-p", "\(serverPort)"])
            defer {
                try? doStop(name: name)
            }
