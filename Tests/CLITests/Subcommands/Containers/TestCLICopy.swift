...
class TestCLICopyCommand: CLITest {
    ...
    @Test func testCopyHostToContainer() throws {
        do {
            ...
            Issue.record("failed to copy file from host to mac-container-tool: \(error)")
            return
        }
    }
    ...
    @Test func testCopyContainerToHost() throws {
        do {
            ...
            let content = "hello from mac-container-tool"
            _ = try doExec(name: name, cmd: ["sh", "-c", "echo -n '\(content)' > /tmp/mac-container-toolfile.txt"])

            let destPath = testDir.appendingPathComponent("mac-container-toolfile.txt")
            let (_, _, error, status) = try run(arguments: [
                "copy",
                "\(name):/tmp/mac-container-toolfile.txt",
                destPath.path,
            ])
            if status != 0 {
                throw CLIError.executionFailed("copy failed: \(error)")
            ...
            Issue.record("failed to copy file from mac-container-tool to host: \(error)")
            return
        }
    }
    ...
    @Test func testCopyContainerToContainerFails() throws {
        do {
            ...
            #expect(status != 0, "expected mac-container-tool-to-mac-container-tool copy to fail")
        } catch {
            Issue.record("failed test for mac-container-tool-to-mac-container-tool copy: \(error)")
            return
        }
    }
    ...
    @Test func testCopyToNonRunningContainerFails() throws {
        do {
            ...
            #expect(status != 0, "expected copy to non-running mac-container-tool to fail")
        } catch {
            Issue.record("failed test for copy to non-running mac-container-tool: \(error)")
            return
        }
    }
    ...
    @Test func testCopyDirectoryHostToContainer() throws {
        do {
            ...
            Issue.record("failed to copy directory from host to mac-container-tool: \(error)")
            return
        }
    }
    ...
    @Test func testCopyDirectoryContainerToHost() throws {
        do {
            ...
            Issue.record("failed to copy directory from mac-container-tool to host: \(error)")
            return
        }
    }
    ...
    @Test func testCopyNestedDirectoryHostToContainer() throws {
        do {
            ...
            Issue.record("failed to copy nested directory from host to mac-container-tool: \(error)")
            return
        }
    }
    ...
    @Test func testCopyNestedDirectoryContainerToHost() throws {
        do {
            ...
            Issue.record("failed to copy nested directory from mac-container-tool to host: \(error)")
            return
        }
    }
    ...
    @Test func testCopyOutFileToExistingFile() throws {
        do {
            ...
            let content = "mac-container-tool content"
