...
public struct Parser {
    ...
    public static func process(
        ...
        guard let commandToRun = processArguments, commandToRun.count > 0 else {
            throw ContainerizationError(.invalidArgument, message: "command/entrypoint not specified for mac-container-tool process")
        }
    ...
    /// The format of each argument is `[host-ip:]host-port:mac-container-tool-port[/protocol]`
    /// (e.g., "127.0.0.1:8080:80/tcp")
    /// host-port and mac-container-tool-port can be ranges (e.g., "127.0.0.1:3456-4567:3456-4567/tcp`
    ///
    ...
    public static func publishPort(_ portText: String) throws -> PublishPort {
        let publishPortRegex = #/((\[(?<ipv6>[^\]]*)\]|(?<ipv4>[^:].*)):)?(?<hostPort>[^:].*):(?<mac-container-toolPort>[^:/]*)(/(?<proto>.*))?/#
        ...
        let mac-container-toolPortText = match.mac-container-toolPort
        let hostPortRangeStart: UInt16
        let hostPortRangeEnd: UInt16
        let mac-container-toolPortRangeStart: UInt16
        let mac-container-toolPortRangeEnd: UInt16
        ...
        let mac-container-toolPortParts = mac-container-toolPortText.split(separator: "-")
        switch mac-container-toolPortParts.count {
        case 1:
            guard let start = UInt16(mac-container-toolPortParts[0]) else {
                throw ContainerizationError(.invalidArgument, message: "invalid publish mac-container-tool port: \(mac-container-toolPortText)")
            }

            mac-container-toolPortRangeStart = start
            mac-container-toolPortRangeEnd = start
        case 2:
            guard let start = UInt16(mac-container-toolPortParts[0]) else {
                throw ContainerizationError(.invalidArgument, message: "invalid publish mac-container-tool port: \(mac-container-toolPortText)")
            }

            guard let end = UInt16(mac-container-toolPortParts[1]) else {
                throw ContainerizationError(.invalidArgument, message: "invalid publish mac-container-tool port: \(mac-container-toolPortText)")
            }

            mac-container-toolPortRangeStart = start
            mac-container-toolPortRangeEnd = end
        default:
            throw ContainerizationError(.invalidArgument, message: "invalid publish mac-container-tool port: \(mac-container-toolPortText)")
        }
        ...
        guard mac-container-toolPortRangeStart > 1,
            mac-container-toolPortRangeStart <= mac-container-toolPortRangeEnd
        else {
            throw ContainerizationError(.invalidArgument, message: "invalid publish mac-container-tool port range: \(mac-container-toolPortText)")
        }

        let hostCount = hostPortRangeEnd - hostPortRangeStart + 1
        let mac-container-toolCount = mac-container-toolPortRangeEnd - mac-container-toolPortRangeStart + 1

        guard hostCount == mac-container-toolCount else {
            throw ContainerizationError(.invalidArgument, message: "publish host and mac-container-tool port counts are not equal: \(hostPortText):\(mac-container-toolPortText)")
        }

        return try PublishPort(
            hostAddress: hostAddress,
            hostPort: hostPortRangeStart,
            mac-container-toolPort: mac-container-toolPortRangeStart,
            proto: proto,
            count: hostCount
        )
    }
    ...
    /// The format of each argument is `host_path:mac-container-tool_path`
    ...
    public static func publishSocket(_ socketText: String) throws -> PublishSocket {
        // Split by colon to two parts: [host_path, mac-container-tool_path]
        let parts = socketText.split(separator: ":")

        switch parts.count {
        case 2:
            // Extract host and mac-container-tool paths
            let hostPath = String(parts[0])
            let mac-container-toolPath = String(parts[1])

            if hostPath.isEmpty {
                throw ContainerizationError(
                    .invalidArgument, message: "host socket path cannot be empty")
            }
            if mac-container-toolPath.isEmpty {
                throw ContainerizationError(
                    .invalidArgument, message: "mac-container-tool socket path cannot be empty")
            }
            ...
            return try PublishSocket(
                mac-container-toolPath: FilePath(mac-container-toolPath),
        ...
        default:
            throw ContainerizationError(
                .invalidArgument,
                message:
                    "invalid publish-socket format \(socketText). Expected: host_path:mac-container-tool_path")
        }
    }
    ...
    public static func capabilities(capAdd: [String], capDrop: [String]) throws -> (capAdd: [String], capDrop: [String]) {
        ...
        for cap in capAdd {
            ...
            // Validate using CapabilityName from the mac-container-toolization lib
