...
class TestCLIBuildBase: CLITest {
    ...
    func waitForBuilderRunning() throws {
        ...
        while attempt > 0 {
            attempt -= 1
            do {
                let response = try doExec(name: buildkitName, cmd: ["pidof", "-s", "mac-container-tool-builder-shim"])
                if !response.isEmpty {
                    // found the init process running
                    return
                }
            } catch {
                print("mac-container-tool-builder-shim check failed with \(error)")
            }
            sleep(1)
        }
        throw CLIError.executionFailed("failed to wait for mac-container-tool-builder-shim process on \(buildkitName)")
    }
