...
extension Application {
    public struct ContainerCopy: AsyncLoggableCommand {
        enum PathRef {
            case local(String)
            case mac-container-tool(id: String, path: String)
        }
        ...
        static func parsePathRef(_ ref: String) throws -> PathRef {
            ...
            case 2 where !parts[0].isEmpty && parts[1].starts(with: "/"):
                return .mac-container-tool(id: parts[0], path: parts[1])
            default:
        ...
        public static let configuration = CommandConfiguration(
            commandName: "copy",
            abstract: "Copy files/folders between a mac-container-tool and the local filesystem",
            aliases: ["cp"])

        @OptionGroup()
        public var logOptions: Flags.Logging

        @Argument(help: "Source path (mac-container-tool:path or local path)")
        var source: String

        @Argument(help: "Destination path (mac-container-tool:path or local path)")
        ...
        public func run() async throws {
            ...
            case (.mac-container-tool(let id, let path), .local(let localPath)):
            ...
            case (.local(let localPath), .mac-container-tool(let id, let path)):
            ...
            case (.mac-container-tool, .mac-container-tool):
                throw ContainerizationError(.invalidArgument, message: "copying between mac-container-tools is not supported")
            case (.local, .local):
                throw ContainerizationError(
                    .invalidArgument,
                    message: "one of source or destination must be a mac-container-tool reference (mac-container-tool_id:path)")
            }
        }
    }
}
