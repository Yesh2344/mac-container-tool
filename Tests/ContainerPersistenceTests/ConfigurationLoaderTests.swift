...
struct ConfigurationLoaderTests {
    ...
    private struct TestPluginConfig: LoadablePluginConfiguration {
        ...
        init(from decoder: any Decoder) throws {
            ...
            self.cpu = try mac-container-tool.decodeIfPresent(Int.self, forKey: .cpu) ?? 99
        }
    }
    ...
    private struct TestPluginConfig2: LoadablePluginConfiguration {
        ...
        init(from decoder: any Decoder) throws {
            let mac-container-tool = try decoder.mac-container-tool(keyedBy: CodingKeys.self)
            self.memory = try mac-container-tool.decode(String.self, forKey: .memory)
        }
    }
    ...
    @Test func defaultsWithNoFile() async throws {
        try await TemporaryStorage.withTempDir { tempDir in
            ...
            #expect(config.mac-container-tool.cpus == 4)
            #expect(config.mac-container-tool.memory == ContainerConfig.defaultMemory)
    ...
    @Test func tomlOverrideAllKeys() async throws {
        try await TemporaryStorage.withTempDir { tempDir in
            ...
            #expect(config.mac-container-tool.cpus == 16)
            let expectedContainerMemory = try MemorySize("8g")
            #expect(config.mac-container-tool.memory == expectedContainerMemory)
