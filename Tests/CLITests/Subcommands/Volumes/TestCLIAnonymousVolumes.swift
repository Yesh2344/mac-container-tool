...
class TestCLIAnonymousVolumes: CLITest {
    ...
    private func cleanUpAllTestResources() {
        // Clean up test mac-container-tools (force remove)
        if let (_, output, _, status) = try? run(arguments: ["ls", "-a"]), status == 0 {
            let mac-container-tools = output.components(separatedBy: .newlines)
                .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
                .filter { $0.lowercased().starts(with: "test") }

            for mac-container-tool in mac-container-tools {
                let _ = (try? run(arguments: ["delete", "--force", mac-container-tool]))
            }
        }
    ...
    @Test func testAnonymousVolumeCreationAndPersistence() async throws {
        ...
        // Run mac-container-tool with --rm and anonymous volume
        ...
        #expect(status == 0, "mac-container-tool run should succeed")

        // Give time for mac-container-tool removal to complete
        try await Task.sleep(for: .seconds(1))

        // Verify mac-container-tool was removed
        let (_, lsOutput, _, _) = try run(arguments: ["ls", "-a"])
        let mac-container-tools = lsOutput.components(separatedBy: .newlines)
            .filter { $0.contains(mac-container-toolName) }
        #expect(mac-container-tools.isEmpty, "mac-container-tool should be removed with --rm")
    ...
    @Test func testAnonymousVolumePersistenceWithoutRm() throws {
        ...
        // Run mac-container-tool WITHOUT --rm
        try doLongRun(name: mac-container-toolName, args: ["-v", "/data"], autoRemove: false)
        try waitForContainerRunning(mac-container-toolName)

        // Write data to anonymous volume
        _ = try doExec(name: mac-container-toolName, cmd: ["sh", "-c", "echo '\(testData)' > /data/test.txt"])
        ...
        // Stop and remove mac-container-tool
        ...
        // Mount same volume in new mac-container-tool and verify data
        let mac-container-toolName2 = "\(testName)_c2"
        try doLongRun(name: mac-container-toolName2, args: ["-v", "\(volumeID):/data"], autoRemove: false)
        try waitForContainerRunning(mac-container-toolName2)

        var output = try doExec(name: mac-container-toolName2, cmd: ["cat", "/data/test.txt"])
        output = output.trimmingCharacters(in: .whitespacesAndNewlines)
        #expect(output == testData, "data should persist in anonymous volume")

        // Clean up
        try doStop(name: mac-container-toolName2)
    ...
    @Test func testMultipleAnonymousVolumes() async throws {
        ...
        // Give time for mac-container-tool removal
        try await Task.sleep(for: .seconds(1))
    ...
    @Test func testAnonymousMountSyntax() async throws {
        ...
        #expect(status == 0, "mac-container-tool run with --mount should succeed")
    ...
    @Test func testAnonymousVolumeUUIDFormat() throws {
        ...
        // Create mac-container-tool with anonymous volume
        try doLongRun(name: mac-container-toolName, args: ["-v", "/data"])
        try waitForContainerRunning(mac-container-toolName)
    ...
    @Test func testAnonymousVolumeManualDeletion() throws {
        ...
        // Create mac-container-tool WITHOUT --rm
        try doLongRun(name: mac-container-toolName, args: ["-v", "/data"], autoRemove: false)
        try waitForContainerRunning(mac-container-toolName)
        ...
        // Stop mac-container-tool (unmounts volume)
        try doStop(name: mac-container-toolName)
        doRemoveIfExists(name: mac-container-toolName, force: true)
    ...
    @Test func testAnonymousVolumeDetachedMode() async throws {
        ...
        #expect(status == 0, "detached mac-container-tool run should succeed")

        // Wait for mac-container-tool to exit
        ...
        let mac-container-tools = lsOutput.components(separatedBy: .newlines)
            .filter { $0.contains(mac-container-toolName) }
        #expect(mac-container-tools.isEmpty, "mac-container-tool should be auto-removed")
