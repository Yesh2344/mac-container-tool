...
extension Application {
    public struct ContainerLogs: AsyncLoggableCommand {
        ...
        public static let configuration = CommandConfiguration(
            commandName: "logs",
            abstract: "Fetch mac-container-tool logs"
        )

        @Flag(name: .long, help: "Display the boot log for the mac-container-tool instead of stdio")
        var boot: Bool = false
        ...
        public func run() async throws {
            ...
            let fhs = try await client.logs(id: mac-container-toolId)
            ...
            if let n {
                ...
                guard let str = String(data: data, encoding: .utf8) else {
                    throw ContainerizationError(
                        .internalError,
                        message: "failed to convert mac-container-tool logs to utf8"
                    )
                }
        ...
        private static func followFile(fh: FileHandle) async throws {
            ...
            let stream = AsyncStream<String> { cont in
                fh.readabilityHandler = { handle in
                    ...
                    if data.isEmpty {
                        // Triggers on mac-container-tool restart - can exit here as well
                        do {
