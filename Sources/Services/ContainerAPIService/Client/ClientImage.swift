...
public struct ClientImage: Sendable {
    ...
    package func resolved() async throws -> Descriptor {
        let index = try await self.index()
        let indirect = index.annotations?[AnnotationKeys.mac-container-toolizationIndexIndirect]
...
extension ClientImage {
    private static let serviceIdentifier = "com.apple.mac-container-tool.core.mac-container-tool-core-images"
...
extension ClientImage {
    ...
    public static func normalizeReference(_ ref: String, mac-container-toolSystemConfig: ContainerSystemConfig) throws -> String {
        guard ref != mac-container-toolSystemConfig.vminit.image else {
            // Don't modify the default init image reference.
            // This is to allow for easier local development against
            // an updated mac-container-toolization.
            return ref
        }
        ...
        if r.domain == nil {
            updatedRawReference = "\(mac-container-toolSystemConfig.registry.domain)/\(ref)"
        }
    ...
    public static func denormalizeReference(_ ref: String, mac-container-toolSystemConfig: ContainerSystemConfig) throws -> String {
        var updatedRawReference: String = ref
        let r = try Reference.parse(ref)
        let defaultRegistry = mac-container-toolSystemConfig.registry.domain
    ...
    public static func get(names: [String], mac-container-toolSystemConfig: ContainerSystemConfig) async throws -> (images: [ClientImage], error: [String]) {
        let all = try await self.list()
        var errors: [String] = []
        var found: [ClientImage] = []
        for name in names {
            do {
                guard let img = try Self._search(reference: name, in: all, mac-container-toolSystemConfig: mac-container-toolSystemConfig) else {
                    errors.append(name)
                    continue
                }
                found.append(img)
    ...
    public static func get(reference: String, mac-container-toolSystemConfig: ContainerSystemConfig) async throws -> ClientImage {
        let all = try await self.list()
        guard let found = try self._search(reference: reference, in: all, mac-container-toolSystemConfig: mac-container-toolSystemConfig) else {
    ...
    private static func _search(reference: String, in all: [ClientImage], mac-container-toolSystemConfig: ContainerSystemConfig) throws -> ClientImage? {
        ...
        let normalizedReference = try Self.normalizeReference(reference, mac-container-toolSystemConfig: mac-container-toolSystemConfig)
    ...
    public static func pull(
        ...
        let reference = try self.normalizeReference(reference, mac-container-toolSystemConfig: mac-container-toolSystemConfig)
        ...
        let insecure = try scheme.schemeFor(host: host, internalDnsDomain: mac-container-toolSystemConfig.dns.domain) == .http
    ...
    public static func save(references: [String], out: String, platform: Platform? = nil, mac-container-toolSystemConfig: ContainerSystemConfig) async throws {
        let (clientImages, errors) = try await get(names: references, mac-container-toolSystemConfig: mac-container-toolSystemConfig)
    ...
    /// - Parameter activeReferences: Set of image references currently in use by mac-container-tools
    ...
    public static func fetch(
        ...
        do {
            let match = try await self.get(reference: reference, mac-container-toolSystemConfig: mac-container-toolSystemConfig)
            if let platform {
            ...
            return try await Self.pull(
                reference: reference, platform: platform, scheme: scheme, mac-container-toolSystemConfig: mac-container-toolSystemConfig, progressUpdate: progressUpdate,
...
extension ClientImage {
    public func push(platform: Platform? = nil, scheme: RequestScheme, mac-container-toolSystemConfig: ContainerSystemConfig, progressUpdate: ProgressUpdateHandler?) async throws {
        let client = Self.newXPCClient()
...
extension ImageDescription {
    fileprivate func nameFromAnnotation() -> String? {
        ...
        guard let name = annotations[AnnotationKeys.mac-container-toolizationImageName] else {
            return nil
        }
        return name
    }
