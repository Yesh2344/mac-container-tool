...
public enum ConfigurationLoader {
    ...
    /// - `.home`: `<base>/config.toml` (user source under `~/.config/mac-container-tool`)
    ///     - e.g. `~/.config/mac-container-tool/config.toml`
    /// - `.appRoot`: `<base>/config/config.toml` (read-only copy of user config)
    ///     - e.g. `~/Library/Application Support/com.apple.mac-container-tool/config/config.toml`
    /// - `.installRoot`: `<base>/etc/mac-container-tool/config.toml` (system defaults shipped with install)
    ///     - e.g. `/usr/local/etc/mac-container-tool/config.toml`
    ///
    /// - Parameters:
    ...
    public static func configurationFile(
        ...
        case .installRoot: base.appending("etc/mac-container-tool").appending(configFilename)
        }
    }
    ...
    /// > system config (`<installRoot>/etc/mac-container-tool/config/config.toml`).
    ///
    ...
    ///
    /// - Parameters:
    ///   - source: File to copy from. Defaults to `<home>/mac-container-tool/config.toml`.
