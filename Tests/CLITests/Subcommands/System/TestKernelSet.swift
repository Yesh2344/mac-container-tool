...
class TestCLIKernelSet: CLITest {
    ...
    @Test func fromLocalTar() async throws {
        let symlinkBinaryPath: String = URL(filePath: defaultBinaryPath).deletingLastPathComponent().appending(path: "vmlinux.mac-container-tool").relativePath
    ...
    @Test func fromRemoteTarSymlink() throws {
        // opt/kata/share/kata-mac-container-tools/vmlinux.mac-container-tool should point to opt/kata/share/kata-mac-container-tools/vmlinux-<version> in the archive
        let symlinkBinaryPath: String = URL(filePath: defaultBinaryPath).deletingLastPathComponent().appending(path: "vmlinux.mac-container-tool").relativePath
        let extraArgs: [String] = [
            "--tar",
