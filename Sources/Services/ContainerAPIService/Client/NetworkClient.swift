...
/// A client for managing virtual networks through the mac-container-tool API server.
///
/// `NetworkClient` communicates with `mac-container-tool-apiserver` over XPC to create,
...
public struct NetworkClient: Sendable {
    /// The Mach service name used to locate the mac-container-tool API server.
    ///
    /// Pass a different value to ``init(serviceIdentifier:)`` to connect to an
    /// alternative service endpoint, for example during testing.
    public static let defaultServiceIdentifier = "com.apple.mac-container-tool.apiserver"

    /// The name of the default network created automatically on first use.
    public static let defaultNetworkName = "default"

    /// The reserved name that indicates a mac-container-tool should have no network attachment.
    ...
    ///
    /// Deletion succeeds only when no mac-container-tools are currently attached to the
