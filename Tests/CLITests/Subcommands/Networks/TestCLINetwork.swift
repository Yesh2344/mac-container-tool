...
class TestCLINetwork: CLITest {
    ...
    @Test func testNetworkCreateAndUse() async throws {
        do {
            ...
            let mac-container-tool = try inspectContainer(name)
            #expect(mac-container-tool.networks.count > 0)
            let cidrAddress = mac-container-tool.networks[0].ipv4Address
    ...
    @Test func testNetworkDeleteWithContainer() async throws {
        do {
            // prep: delete mac-container-tool and network, ignoring if it doesn't exist
            let name = getLowercasedTestName()
            ...
            // create a mac-container-tool that refers to the network
            ...
            // now get rid of the mac-container-tool and its network reference
            try? doRemove(name: name)
