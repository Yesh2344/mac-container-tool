...
struct UnkeyedContainer: UnkeyedDecodingContainer {
    ...
    mutating func nestedContainer<NestedKey: CodingKey>(
        ...
        throw DecodingError.dataCorrupted(
            DecodingError.Context(
                codingPath: codingPath,
                debugDescription: "ConfigSnapshotDecoder does not support nested mac-container-tools inside arrays."
            )
        )
    }
    ...
    private func checkBounds(_ count: Int) throws {
        guard currentIndex < count else {
            throw DecodingError.valueNotFound(
                ...
                DecodingError.Context(
                    codingPath: codingPath,
                    debugDescription: "Unkeyed mac-container-tool is at end (index \(currentIndex), count \(count))."
                )
            )
        }
    }
