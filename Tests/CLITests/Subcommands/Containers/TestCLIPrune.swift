...
class TestCLIPruneCommand: CLITest {
    ...
    @Test func testContainerPruneNoContainers() throws {
        ...
        if status != 0 {
            throw CLIError.executionFailed("mac-container-tool prune failed: \(error)")
        }

        #expect(error.contains("Reclaimed Zero KB in disk space"), "should show no mac-container-tools message")
    }
    ...
    @Test func testContainerPruneStoppedContainers() throws {
        ...
        try doLongRun(name: npcName, mac-container-toolArgs: ["sleep", "3600"], autoRemove: true)
        try doLongRun(name: pc0Name, mac-container-toolArgs: ["sleep", "3600"], autoRemove: false)
        try doLongRun(name: pc1Name, mac-container-toolArgs: ["sleep", "3600"], autoRemove: false)
        defer {
        ...
        // Poll status until both mac-container-tools are stopped, with interval checks and a timeout to avoid infinite loop
        let start = Date()
        ...
        while true {
            ...
            if Date().timeIntervalSince(start) > timeout {
                throw CLIError.executionFailed("Timeout waiting for mac-container-tools to stop: pc0=\(s0), pc1=\(s1)")
            }
        ...
        #expect(output.contains(pc0Id) && output.contains(pc1Id), "should show the stopped mac-container-tools id")
        ...
        #expect(checkStatus == "running", "not pruned mac-container-tool should still be running")
    }
}
