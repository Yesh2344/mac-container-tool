...
struct TestCLIPluginErrors {
    ...
    func testHelpfulMessageWhenPluginsUnavailable() throws {
        ...
        #expect(status != 0)
        #expect(stderr.contains("mac-container-tool system start"))
        #expect(stderr.contains("Plugins are unavailable") || stderr.contains("Plugin 'mac-container-tool-"))
        // Should include at least one computed plugin search path hint
        #expect(stderr.contains("mac-container-tool-plugins") || stderr.contains("mac-container-tool/plugins"))
    }
}
