...
public actor VolumesService {
    ...
    public init(resourceRoot: FilePath, mac-container-toolsService: ContainersService, log: Logger) async throws {
    ...
    public func calculateDiskUsage() async throws -> (Int, Int, UInt64, UInt64) {
        ...
        return try await lock.withLock { _ in
            ...
            // Atomically get active volumes with mac-container-tool list
            return try await self.mac-container-toolsService.withContainerList(logMetadata: ["acquirer": "\(#function)"]) { mac-container-tools in
                var inUseSet = Set<String>()

                // Find all mounted volumes
                for mac-container-tool in mac-container-tools {
                    for mount in mac-container-tool.configuration.mounts {
    ...
    private func createVolumeImage(for name: String, sizeInBytes: UInt64 = VolumeStorage.defaultVolumeSizeBytes, journal: EXT4.JournalConfig? = nil) throws {
        ...
        // Use the mac-container-toolization library's EXT4 formatter
    ...
    private func _delete(name: String) async throws {
        ...
        // Check if volume is in use by any mac-container-tool atomically
        try await mac-container-toolsService.withContainerList(logMetadata: ["acquirer": "\(#function)", "name": "\(name)"]) { mac-container-tools in
            for mac-container-tool in mac-container-tools {
                for mount in mac-container-tool.configuration.mounts {
