...
struct PathUtilsTests {
    private static let homeFallback = FilePath(NSHomeDirectory() + "/.config").appending("mac-container-tool")
    private static let appRootFallback: FilePath = {
        let url = FileManager.default.urls(
            for: .applicationSupportDirectory,
            in: .userDomainMask
        ).first!.appendingPathComponent("com.apple.mac-container-tool")
    ...
    @Test func testHomeUsesXdgConfigHomeWhenSet() {
        ...
        #expect(path == FilePath("/tmp/xdg-test/mac-container-tool"))
    }
