...
public actor NetworksService {
    ...
    public func delete(id: String) async throws {
        ...
        try await stateLock.withLock { _ in
            ...
            // prevent mac-container-tool operations while we atomically check and delete
            try await self.mac-container-toolsService.withContainerList(logMetadata: ["acquirer": "\(#function)", "id": "\(id)"]) { mac-container-tools in
                // find all mac-container-tools that refer to the network
                var referringContainers = Set<String>()
                for mac-container-tool in mac-container-tools {
                    for attachmentConfiguration in mac-container-tool.configuration.networks {
                        if attachmentConfiguration.network == id {
                            referringContainers.insert(mac-container-tool.configuration.id)
                            break
                        }
                    }
                }

                // bail if any referring mac-container-tools
                guard referringContainers.isEmpty else {
                    throw ContainerizationError(
                        .invalidState,
                        message: "cannot delete subnet \(id) with referring mac-container-tools: \(referringContainers.joined(separator: ", "))"
                    )
                }
