...
private nonisolated(unsafe) var bootstrapLogger = {
    ...
    var log = Logger(label: "com.apple.mac-container-tool")
    log.logLevel = .info
    return log
}()
...
public struct Application: AsyncLoggableCommand {
    ...
    public static let configuration = CommandConfiguration(
        commandName: "mac-container-tool",
        abstract: "A mac-container-tool platform for macOS",
        version: ReleaseVersion.singleLine(appName: "mac-container-tool CLI"),
        subcommands: [
            DefaultCommand.self,
            HelpCommand.self,
        ],
    ...
    public static func main() async throws {
        ...
        do {
            ...
            // --help/-h on the root command (e.g. `mac-container-tool --help`) is intercepted
            ...
            if errorAsString.contains("XPC connection error") {
                let modifiedError = ContainerizationError(.interrupted, message: "\(error)\nEnsure mac-container-tool system service has been started with `mac-container-tool system start`.")
    ...
    /// daemon. `mac-container-tool system start` MUST have previously been run to start the daemon.
    ...
    public func validate() throws {
        ...
        if try isTranslated() {
            throw ValidationError(
                """
                `mac-container-tool` is currently running under Rosetta Translation, which could be
...
extension Application {
    ...
    static func printModifiedHelpText(pluginLoader: PluginLoader?) async {
        ...
        guard let pluginLoader else {
            ...
            print("\nPLUGINS: not available, run `mac-container-tool system start`")
            return
        }
