...
class TestCLIMachineCommand: CLITest {
    ...
    @Test func testCreate() throws {
        ...
        #expect(throws: Never.self, "expected mac-container-tool machine create to succeed") {
...
/// Integration tests for mac-container-tool machine runtime commands: stop, inspect, run, set.
/// Tests are serialized since mac-container-tool machine operations share system resources and
/// concurrent VM operations could interfere with each other.
@Suite(.serialSuites, .serialized)
class TestCLIMachineRuntime: CLITest {
    let machineImage = "ghcr.io/linuxmac-container-tools/alpine:3.20"
    ...
    private func doMachineCreate(name: String, image: String? = nil) throws {
        ...
        if status != 0 {
            throw CLIError.executionFailed("mac-container-tool machine create failed: \(error)")
        }
    }
    ...
    private func doMachineBoot(name: String? = nil) throws -> String {
        ...
        let (_, _, error, status) = try runMachine(arguments: args)
        if status != 0 {
            throw CLIError.executionFailed("mac-container-tool machine boot (via run) failed: \(error)")
        }
        // Return the mac-container-tool machine name for compatibility with existing tests
        return name ?? ""
    }
    ...
    private func doMachineStop(name: String? = nil) throws -> String {
        ...
        if status != 0 {
            throw CLIError.executionFailed("mac-container-tool machine stop failed: \(error)")
        }
    ...
    private func doMachineInspect(name: String? = nil) throws -> MachineInspectOutput {
        ...
        if status != 0 {
            throw CLIError.executionFailed("mac-container-tool machine inspect failed: \(error)")
        }
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        let results = try decoder.decode([MachineInspectOutput].self, from: outputData)
        guard let result = results.first else {
            throw CLIError.executionFailed("mac-container-tool machine inspect returned empty array")
        }
        return result
    }
    ...
    private func doMachineRun(
        ...
        let (_, output, error, status) = try runMachine(arguments: args)
        if status != 0 {
            throw CLIError.executionFailed("mac-container-tool machine run failed: \(error)")
        }
        return output
    }
    ...
    private func doMachineRemove(name: String) throws {
        ...
        let (_, _, error, status) = try runMachine(arguments: args)
        if status != 0 {
            throw CLIError.executionFailed("mac-container-tool machine rm failed: \(error)")
        }
    }
    ...
    private func waitForMachineStatus(_ name: String, status: String, maxAttempts: Int = 30) throws {
        ...
        throw CLIError.executionFailed("mac-container-tool machine \(name) did not reach status \(status)")
    }
    ...
    @Test func testCreateNameLongestValid() throws {
        let maxNameLength = LinuxContainer.maxIDLength - MachineConfiguration.mac-container-toolUUIDLength - 1
    ...
    @Test func testStopRunningMachine() throws {
        ...
        #expect(beforeSnapshot.startedDate != nil, "running mac-container-tool machine should have startedDate")

        let output = try doMachineStop(name: name)
        #expect(output == name, "stop should output the mac-container-tool machine ID")

        let afterSnapshot = try doMachineInspect(name: name)
        #expect(afterSnapshot.status == "stopped", "mac-container-tool machine should be stopped after stop")
        #expect(afterSnapshot.startedDate == nil, "stopped mac-container-tool machine should not have startedDate")
    }

    /// Verifies that stopping an already-stopped mac-container-tool machine is a no-op and succeeds.
    @Test func testStopIdempotent() throws {
        ...
        let output = try doMachineStop(name: name)
        #expect(output == name, "stop on stopped mac-container-tool machine should succeed")

        let snapshot = try doMachineInspect(name: name)
        #expect(snapshot.status == "stopped", "mac-container-tool machine should remain stopped")
    }
    ...
    @Test func testStopNonExistentMachine() throws {
        ...
        #expect(status != 0, "stop should fail for non-existent mac-container-tool machine")
    ...
    @Test func testInspectStoppedMachine() throws {
        ...
        let snapshot = try doMachineInspect(name: name)
        #expect(snapshot.status == "stopped", "new mac-container-tool machine should be stopped")
        #expect(snapshot.startedDate == nil, "stopped mac-container-tool machine should not have startedDate")
    ...
    @Test func testInspectRunningMachine() throws {
        ...
        let snapshot = try doMachineInspect(name: name)
        #expect(snapshot.status == "running", "booted mac-container-tool machine should be running")
        #expect(snapshot.startedDate != nil, "running mac-container-tool machine should have startedDate")
        #expect(snapshot.id == name, "configuration should have correct ID")
    ...
    @Test func testInspectNonExistentMachine() throws {
        ...
        #expect(status != 0, "inspect should fail for non-existent mac-container-tool machine")
        #expect(error.contains("not found"), "error should mention 'not found'")
    }
    ...
    /// Verifies that running a command on a stopped mac-container-tool machine automatically boots it.
    ...
    @Test func testRunAutoBoots() throws {
        ...
        let beforeSnapshot = try doMachineInspect(name: name)
        #expect(beforeSnapshot.status == "stopped", "mac-container-tool machine should start stopped")
        ...
        let afterSnapshot = try doMachineInspect(name: name)
        #expect(afterSnapshot.status == "running", "mac-container-tool machine should be running after auto-boot")
    }
    ...
    @Test func testRunNonExistentMachine() throws {
        ...
        #expect(status != 0, "run should fail for non-existent mac-container-tool machine")
        #expect(error.contains("not found"), "error should mention 'not found'")
    }

    /// End-to-end test covering the full mac-container-tool machine lifecycle including re-boot after stop.
    @Test func testFullLifecycle() throws {
        ...
        #expect(snapshot.status == "running", "mac-container-tool machine should be running after boot")
        ...
        snapshot = try doMachineInspect(name: name)
        #expect(snapshot.status == "stopped", "mac-container-tool machine should be stopped after stop")

        _ = try doMachineBoot(name: name)
        try waitForMachineStatus(name, status: "running")

        snapshot = try doMachineInspect(name: name)
        #expect(snapshot.status == "running", "mac-container-tool machine should be running after re-boot")
    }
    ...
    @Test func testListRunningMachines() throws {
        ...
        #expect(output.contains(name), "list should show running mac-container-tool machine")
    }
    ...
    @Test func testListAllMachines() throws {
        ...
        let (_, output, _, status) = try runMachine(arguments: ["ls"])
        #expect(status == 0, "list should succeed")
        #expect(output.contains(name), "stopped mac-container-tool machine should appear in list")
    }
    ...
    @Test func testListEmpty() throws {
        // List with no running mac-container-tool machines
        let (_, output, _, status) = try runMachine(arguments: ["ls"])
        #expect(status == 0, "list should succeed even with no running mac-container-tool machines")
    ...
    @Test func testSetDefault() throws {
        ...
        #expect(output.trimmingCharacters(in: .whitespacesAndNewlines) == name, "should output the mac-container-tool machine ID")

        // Verify it's now the default by listing
        let (_, listOutput, _, _) = try runMachine(arguments: ["ls"])
        #expect(listOutput.contains("*"), "mac-container-tool machine should be marked as default in list")
    }
    ...
    @Test func testSetDefaultNonExistent() throws {
        ...
        #expect(status != 0, "set default should fail for non-existent mac-container-tool machine")
        #expect(error.contains("not found"), "error should mention 'not found'")
    }
    ...
    @Test func testInspectUsesDefault() throws {
        ...
        let snapshot = try doMachineInspect()
        #expect(snapshot.id == name, "inspect should use default mac-container-tool machine")
    }
    ...
    /// Verifies that killing a mac-container-tool machine's backing mac-container-tool updates the mac-container-tool machine state to stopped.
    @Test func testContainerExitState() throws {
        ...
        let snapshot = try doMachineInspect(name: name)
        guard let mac-container-toolId = snapshot.mac-container-toolId else {
            throw CLIError.executionFailed("running mac-container-tool machine has no mac-container-toolId")
        }

        try doStop(name: mac-container-toolId)
        try waitForMachineStatus(name, status: "stopped")

        let after = try doMachineInspect(name: name)
        #expect(after.status == "stopped", "mac-container-tool machine should be stopped after mac-container-tool is killed")
        #expect(after.mac-container-toolId == nil, "stopped mac-container-tool machine should have no mac-container-toolId")
        #expect(after.startedDate == nil, "stopped mac-container-tool machine should have no startedDate")
    }
    ...
    @Test func testSSHForwarding() throws {
        ...
        // Boot the mac-container-tool machine with SSH_AUTH_SOCK set in the process environment.
        let (_, _, bootError, bootStatus) = try runMachine(
            arguments: ["run", "--root", "-n", name, "true"],
            env: ["SSH_AUTH_SOCK": socketPath]
        )
        if bootStatus != 0 {
            throw CLIError.executionFailed("mac-container-tool machine boot with SSH_AUTH_SOCK failed: \(bootError)")
        }
        try waitForMachineStatus(name, status: "running")
        ...
        #expect(
            ...
            "expected forwarded SSH socket to exist in mac-container-tool machine"
        )
    }
    ...
    /// Verifies that `mac-container-tool machine create` boots the machine by default.
    ...
    @Test func testLogs() throws {
        ...
        #expect(bootStatus == 0, "logs --boot should succeed for stopped mac-container-tool machine")
        #expect(
        ...
        #expect(stdioStatus == 0, "logs should succeed for stopped mac-container-tool machine")
    }
    ...
    @Test func testLogsWhileRunning() throws {
        ...
        let (_, bootOutput, _, bootStatus) = try runMachine(arguments: ["logs", "--boot", name])
        #expect(bootStatus == 0, "logs --boot should succeed while mac-container-tool machine is running")
        #expect(
            !bootOutput.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
        ...
        let (_, _, _, stdioStatus) = try runMachine(arguments: ["logs", name])
        #expect(stdioStatus == 0, "logs should succeed while mac-container-tool machine is running")
    }
    ...
    @Test func testMachineNotInContainerList() throws {
        ...
        let inspect = try doMachineInspect(name: name)
        let mac-container-toolId = try #require(inspect.mac-container-toolId, "machine should have an underlying mac-container-tool ID")

        let (_, runningOutput, _, runningStatus) = try run(arguments: ["ls", "-q"])
        #expect(runningStatus == 0, "mac-container-tool ls failed")
        #expect(!runningOutput.contains(mac-container-toolId), "machine mac-container-tool should not appear in 'mac-container-tool ls'")

        let (_, allOutput, _, allStatus) = try run(arguments: ["ls", "-a", "-q"])
        #expect(allStatus == 0, "mac-container-tool ls -a failed")
        #expect(!allOutput.contains(mac-container-toolId), "machine mac-container-tool should not appear in 'mac-container-tool ls -a'")
    }
    ...
    @Test func testMachineNotDeletedByRmAll() throws {
        ...
        let inspect = try doMachineInspect(name: name)
        let mac-container-toolId = try #require(inspect.mac-container-toolId, "running machine should have an underlying mac-container-tool ID")

        let (_, _, deleteError, deleteStatus) = try run(arguments: ["delete", "--all"])
        #expect(deleteStatus == 0, "mac-container-tool delete --all failed: \(deleteError)")

        let mac-container-toolInspect = try inspectContainer(mac-container-toolId)
        #expect(mac-container-toolInspect.status.state == "running", "machine mac-container-tool should still be running after 'mac-container-tool delete --all'")
    }
    ...
    @Test func testMachineNotKilledByKillAll() throws {
        ...
        #expect(killStatus == 0, "mac-container-tool kill --all failed: \(killError)")

        let mac-container-toolInspect = try inspectContainer(mac-container-toolId)
        #expect(mac-container-toolInspect.status.state == "running", "machine mac-container-tool should still be running after 'mac-container-tool kill --all'")
    }
    ...
    @Test func testMachineNotStoppedByStopAll() throws {
        ...
        #expect(stopStatus == 0, "mac-container-tool stop --all failed: \(stopError)")

        let mac-container-toolInspect = try inspectContainer(mac-container-toolId)
        #expect(mac-container-toolInspect.status.state == "running", "machine mac-container-tool should still be running after 'mac-container-tool stop --all'")
    }
}
...
struct MachineInspectOutput: Codable {
    ...
    let createdDate: Date?
    let mac-container-toolId: String?
    let cpus: Int
    let memory: UInt64
