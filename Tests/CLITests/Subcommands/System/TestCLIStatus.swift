...
/// Tests for `mac-container-tool system status` output formats and content validation.
@Suite(.serialSuites)
final class TestCLIStatus: CLITest {
    ...
    @Test func prefixOption() throws {
        ...
        let (_, out, _, status) = try run(arguments: ["system", "status", "--prefix", "com.apple.mac-container-tool."])
