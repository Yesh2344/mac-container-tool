    ...
    class CLIBuilderLifecycleTest: TestCLIBuildBase {
        ...
        @Test func testBuilderStartStopCommand() throws {
            #expect(throws: Never.self) {
                ...
                #expect(status == "running", "BuildKit mac-container-tool is not running")
            }
            #expect(throws: Never.self) {
                try self.builderStop()
                let status = try self.getContainerStatus("buildkit")
                #expect(status == "stopped", "BuildKit mac-container-tool is not stopped")
            }
        }
        ...
        @Test func testBuilderEnvironmentColors() throws {
            ...
            let mac-container-tool = try inspectContainer("buildkit")
            let envVars = mac-container-tool.configuration.initProcess.environment

            #expect(
                envVars.contains("BUILDKIT_COLORS=\(testColors)"),
                "Expected BUILDKIT_COLORS to be passed to mac-container-tool, but it was missing from env: \(envVars)"
            )
            #expect(
                envVars.contains("NO_COLOR=\(testNoColor)"),
                "Expected NO_COLOR to be passed to mac-container-tool, but it was missing from env: \(envVars)"
            )
        }
    }
}
