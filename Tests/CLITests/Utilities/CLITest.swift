...
class CLITest {
    ...
    init() throws {
        ...
        let logger = Logger(label: "com.apple.mac-container-tool.test") { label in
    ...
    let alpine = "ghcr.io/linuxmac-container-tools/alpine:3.20"
    let alpine318 = "ghcr.io/linuxmac-container-tools/alpine:3.18"
    let busybox = "ghcr.io/mac-container-toold/busybox:1.36"

    let defaultContainerArgs = ["sleep", "infinity"]

    var executablePath: URL {
        get throws {
            let mac-container-toolPath = ProcessInfo.processInfo.environment["CONTAINER_CLI_PATH"]
            if let mac-container-toolPath {
                return URL(filePath: mac-container-toolPath)
            }
            ...
            let binURL = URL(fileURLWithPath: currentDir)
                ...
                .appendingPathComponent("mac-container-tool")
    ...
    func waitForContainerRunning(_ name: String, _ totalAttempts: Int64 = 100) throws {
        ...
        if !found {
            throw CLIError.mac-container-toolNotFound(name)
        }
    }
    ...
    enum CLIError: Error {
        ...
        case mac-container-toolNotFound(String)
        case mac-container-toolRunFailed(String)
        case binaryNotFound
    }
    ...
    func doLongRun(
        ...
        image: String? = nil,
        args: [String]? = nil,
        mac-container-toolArgs: [String]? = nil,
        ...
        if let mac-container-toolArgs {
            runArgs.append(contentsOf: mac-container-toolArgs)
        } else {
    ...
    func inspectContainer(_ name: String) throws -> inspectOutput {
        ...
        guard cmdStatus == 0 else {
            throw CLIError.executionFailed("mac-container-tool inspect failed: exit \(cmdStatus)")
        }

        let output = response.output
        guard let jsonData = output.data(using: .utf8) else {
            throw CLIError.invalidOutput("mac-container-tool inspect output invalid")
        }
