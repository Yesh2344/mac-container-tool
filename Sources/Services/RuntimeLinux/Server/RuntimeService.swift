...
/// An XPC service that manages the lifecycle of a single VM-backed mac-container-tool.
public actor RuntimeService {
    private let connection: xpc_connection_t
    private let root: URL
    private let interfaceStrategies: [NetworkInterfaceKey: InterfaceStrategy]
    private var mac-container-tool: ContainerInfo?
    ...
    /// Start the VM and the guest agent process for a mac-container-tool.
    ///
    /// - Parameters:
    ///   - message: An XPC message with no parameters.
    ///
        ...
        return try await self.lock.withLock { _ in
            guard await self.state == .created else {
                throw ContainerizationError(
                    .invalidState,
                    message: "mac-container-tool expected to be in created state, got: \(await self.state)"
                )
            }
            ...
            let mac-container-toolLog = try FileHandle(forWritingTo: bundle.mac-container-toolLog)
            let stdout = {
                if let h = stdio[1] {
                    return MultiWriter(handles: [h, mac-container-toolLog])
                }
                return MultiWriter(handles: [mac-container-toolLog])
            }()
            ...
            let id = config.id
            let rootfs = try bundle.mac-container-toolRootfs.asMount
            let mac-container-tool = try LinuxContainer(id, rootfs: rootfs, vmm: vmm, logger: self.log) { czConfig in
            ...
            let ctrInfo = ContainerInfo(
                mac-container-tool: mac-container-tool,
                config: config,
            ...
            do {
                ...
                do {
                    try await self.cleanUpContainer(mac-container-toolInfo: ctrInfo)
                    await self.setState(.stopped)
                } catch {
                    self.log.error("failed to clean up mac-container-tool", metadata: ["error": "\(error)"])
                }
                throw error
            }
    ...
    /// Start the mac-container-tool workload inside the virtual machine.
    ///
    /// - Parameters:
    ...
    public func startProcess(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        return try await self.lock.withLock { lock in
            ...
            let mac-container-toolInfo = try await self.getContainer()
            let mac-container-toolId = mac-container-toolInfo.mac-container-tool.id
            if id == mac-container-toolId {
    ...
    public func statistics(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        return try await self.lock.withLock { lock in
            let mac-container-toolInfo = try await self.getContainer()
            let stats = try await mac-container-toolInfo.mac-container-tool.statistics()

            let mac-container-toolStats = ContainerStats(
                id: stats.id,
            ...
            let reply = message.reply()
            let data = try JSONEncoder().encode(mac-container-toolStats)
    ...
    public func shutdown(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        return try await self.lock.withLock { _ in
            ...
            default:
                throw ContainerizationError(
                    .invalidState,
                    message: "cannot shutdown: mac-container-tool is not stopped"
                )
            }
    ...
    /// Create a process inside the virtual machine for the mac-container-tool.
    ///
    /// Use this procedure to run ad hoc processes in the virtual
    /// machine (`mac-container-tool exec`).
    ///
    /// - Parameters:
    ///   - message: An XPC message with the following parameters:
    ///     - id: A client identifier for the process.
    ...
    public func createProcess(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        return try await self.lock.withLock { [self] _ in
            ...
            default:
                throw ContainerizationError(
                    .invalidState,
                    message: "cannot exec: mac-container-tool is not running"
                )
            }
        }
    }

    /// Return the state for the sandbox and its mac-container-tools.
    ///
    /// - Parameters:
    ///   - message: An XPC message with no parameters.
    ///
    ...
    public func state(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        try reply.setState(
            .init(
                status: status,
                networks: networks,
                mac-container-tools: cs != nil ? [cs!] : []
            )
        )
        return reply
    }

    /// Stop the mac-container-tool workload, any ad hoc processes, and the underlying
    /// virtual machine.
    ///
    /// - Parameters:
    ///   - message: An XPC message with the following parameters:
        ...
        return try await self.lock.withLock { _ in
            ...
            case .running, .booted:
                ...
                do {
                    ...
                    try await self.cleanUpContainer(mac-container-toolInfo: ctr, exitStatus: exitStatus)
                } catch {
                    self.log.error("failed to clean up mac-container-tool", metadata: ["error": "\(error)"])
                }
                await self.setState(.stopped)
    ...
    public func kill(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        try await self.lock.withLock { [self] _ in
            ...
            case .running:
                ...
                if id != ctr.mac-container-tool.id {
                ...
                try await ctr.mac-container-tool.kill(signal)
            default:
                throw ContainerizationError(
                    .invalidState,
                    message: "cannot kill: mac-container-tool is not running"
                )
            }
        }
    ...
    public func resize(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        case .running:
            ...
            if id != ctr.mac-container-tool.id {
                ...
                try await ctr.mac-container-tool.resize(
                    to: .init(
                        width: UInt16(width),
                        height: UInt16(height))
                )
        ...
        default:
            throw ContainerizationError(
                .invalidState,
                message: "cannot resize: mac-container-tool is not running"
            )
        }
    }
    ...
    ///     - destinationPath: The mac-container-tool path to copy to.
    ...
    public func copyIn(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        case .running, .booted:
            ...
            let ctr = try getContainer()
            try await ctr.mac-container-tool.copyIn(
        ...
        default:
            throw ContainerizationError(
                .invalidState,
                message: "cannot copyIn: mac-container-tool is not running"
            )
        }
    }
    ...
    /// - Parameters:
    ///   - message: An XPC message with the following parameters:
    ///     - sourcePath: The mac-container-tool path to copy from.
    ...
    public func copyOut(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        case .running, .booted:
            ...
            let ctr = try getContainer()
            try await ctr.mac-container-tool.copyOut(
                from: URL(fileURLWithPath: source),
                to: URL(fileURLWithPath: destination),
                createParents: createParents
            )
        ...
        default:
            throw ContainerizationError(
                .invalidState,
                message: "cannot copyOut: mac-container-tool is not running"
            )
        }
    }
    ...
    public func dial(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        case .running, .booted:
            ...
            let ctr = try getContainer()
            let fh = try await ctr.mac-container-tool.dialVsock(port: UInt32(port))
        ...
        default:
            throw ContainerizationError(
                .invalidState,
                message: "cannot dial: mac-container-tool is not running"
            )
        }
    }
    ...
    private func startInitProcess(lock: AsyncLock.Context) async throws {
        ...
        let mac-container-tool = info.mac-container-tool
        let id = mac-container-tool.id

        guard self.state == .booted else {
            throw ContainerizationError(
                .invalidState,
                message: "mac-container-tool expected to be in booted state, got: \(self.state)"
            )
        }
        ...
        do {
            ...
            try await mac-container-tool.start()
            let waitFunc: ExitMonitor.WaitHandler = {
                let code = try await mac-container-tool.wait()
                if let out = io.out {
                    try out.close()
                }
                if let err = io.err {
            ...
            try? await self.cleanUpContainer(mac-container-toolInfo: info)
    ...
    private func startExecProcess(processId id: String, lock: AsyncLock.Context) async throws {
        let mac-container-tool = try self.getContainer().mac-container-tool
        guard let processInfo = self.processes[id] else {
            throw ContainerizationError(.notFound, message: "process with id \(id)")
        }

        let mac-container-toolInfo = try self.getContainer()
        let czConfig = try self.configureProcessConfig(
            config: processInfo.config,
            stdio: processInfo.io,
            mac-container-toolConfig: mac-container-toolInfo.config,
        )

        let process = try await mac-container-tool.exec(id, configuration: czConfig)
        ...
        try await withThrowingTaskGroup(of: SocketForwarderResult.self) { group in
            for publishedPort in publishedPorts {
                for index in 0..<publishedPort.count {
                    ...
                    let mac-container-toolIPAddress: String
                    switch publishedPort.hostAddress {
                    case .v4(_):
                        mac-container-toolIPAddress = attachment.ipv4Address.address.description
                    case .v6(_):
                        guard let ipv6Address = attachment.ipv6Address else {
                            throw ContainerizationError(.invalidState, message: "cannot configure IPv6 port forwarding for mac-container-tool with unknown IPv6 address")
                        }
                        mac-container-toolIPAddress = ipv6Address.address.description
                    }
                    let serverAddress = try SocketAddress(ipAddress: mac-container-toolIPAddress, port: Int(publishedPort.mac-container-toolPort + index))
                    log.info(
    ...
    private func onContainerExit(id: String, exitStatus: ExitStatus) async throws {
        ...
        try await self.lock.withLock { [self] _ in
            ...
            do {
                try await cleanUpContainer(mac-container-toolInfo: ctrInfo, exitStatus: exitStatus)
            } catch {
                self.log.error("failed to clean up mac-container-tool", metadata: ["error": "\(error)"])
            }
    ...
    private static func configureContainer(
        ...
        // If the host doesn't support this, we'll throw on mac-container-tool creation.
        ...
        for publishedSocket in config.publishedSockets {
            ...
            let socketConfig = UnixSocketConfiguration(
                source: URL(filePath: publishedSocket.mac-container-toolPath.string),
    ...
    private nonisolated func configureProcessConfig(config: ProcessConfiguration, stdio: [FileHandle?], mac-container-toolConfig: ContainerConfiguration)
        throws -> LinuxProcessConfiguration
    {
        ...
        if mac-container-toolConfig.ssh {
        ...
        proc.capabilities = try Self.effectiveCapabilities(
            capAdd: mac-container-toolConfig.capAdd,
            capDrop: mac-container-toolConfig.capDrop
        )
        switch config.user {
        case .raw(let name):
            proc.user = .init(
    ...
    private func getContainer() throws -> ContainerInfo {
        guard let mac-container-tool else {
            throw ContainerizationError(
                .invalidState,
                message: "no mac-container-tool found"
            )
        }
        ...
        do {
            code = try await withThrowingTaskGroup(of: ExitStatus.self) { group in
                ...
                guard let code = try await group.next() else {
                    throw ContainerizationError(
                        .internalError,
                        message: "failed to get exit code from gracefully stopping mac-container-tool"
                    )
                }
                group.cancelAll()
    ...
    private func cleanUpContainer(mac-container-toolInfo: ContainerInfo, exitStatus: ExitStatus? = nil) async throws {
        let mac-container-tool = mac-container-toolInfo.mac-container-tool
        let id = mac-container-tool.id

        do {
            try await mac-container-tool.stop()
        } catch {
            self.log.error("failed to stop mac-container-tool during cleanup", metadata: ["error": "\(error)"])
        }
...
extension ContainerResource.Bundle {
    func createLogFile() throws {
        ...
        // O_TRUNC resolves a log delay issue on restarted mac-container-tools by force-updating internal state
        let fd = Darwin.open(self.mac-container-toolLog.path, O_CREAT | O_RDONLY | O_TRUNC, 0o644)
        guard fd > 0 else {
...
extension RuntimeService {
    ...
    private struct ContainerInfo {
        let mac-container-tool: LinuxContainer
    ...
    private func createBundle() throws {
        do {
            ...
            _ = try ContainerResource.Bundle.create(
                ...
                mac-container-toolConfiguration: runtimeConfig.mac-container-toolConfiguration,
                mac-container-toolRootFilesystem: runtimeConfig.mac-container-toolRootFilesystem,
