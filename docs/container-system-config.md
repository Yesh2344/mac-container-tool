...
For a guided walk-through on setting default values, see [Container system config tutorial](./tutorials/mac-container-tool-system-config-tutorial.md). 
...
[mac-container-tool]  # default per-mac-container-tool resources
...
Resources and image used for the builder VM that runs `mac-container-tool build`.
...
| `image`   | `String`    | `ghcr.io/apple/mac-container-tool-builder-shim/builder:<tag>` | Reference for the builder image. The tag segment is taken from the project's bundled `mac-container-tool-builder-shim` version. |

## `[mac-container-tool]`

Defaults applied when `mac-container-tool run` / `mac-container-tool create` is invoked without `--cpus` or `--memory`.

| Key      | Type       | Default | Description                                                                |
|----------|------------|---------|----------------------------------------------------------------------------|
| `cpus`   | `Int`      | `4`     | Default CPU count per mac-container-tool.                                           |
| `memory` | [MemorySize](#memorysize-format) | `"1g"`  | Default RAM per mac-container-tool. |

## `[dns]`

| Key      | Type      | Default | Description                                                                |
|----------|-----------|---------|----------------------------------------------------------------------------|
| `domain` | `String?` | unset   | Local DNS domain appended to mac-container-tool hostnames (e.g. `"test"` makes `my-web-server` resolvable as `my-web-server.test`). When unset, no domain is appended. |

## `[kernel]`

Guest kernel used when launching mac-container-tool VMs. Defaults change per release as kernels are bumped — check the [source](../Sources/ContainerPersistence/ContainerSystemConfig.swift) for current values.

| Key          | Type     | Default                                                                                                | Description                                                                  |
|--------------|----------|--------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------|
| `binaryPath` | `String` | `"opt/kata/share/kata-mac-container-tools/vmlinux-6.18.15-186"`                                                 | Path **inside** the downloaded kernel archive that points to the kernel binary. |
| `url`        | `URL`    | `"https://github.com/kata-mac-container-tools/kata-mac-container-tools/releases/download/3.28.0/kata-static-3.28.0-arm64.tar.zst"` | Archive to download when no kernel is installed. Encoded and decoded as a plain string in TOML. |
...
| Key     | Type     | Default                                                | Description                                                                                              |
|---------|----------|--------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| `image` | `String` | `ghcr.io/apple/mac-container-toolization/vminit:<tag>`     | Reference for the `vminitd` image used to boot mac-container-tool VMs. The tag segment is taken from the project's bundled `mac-container-toolization` version.  |
