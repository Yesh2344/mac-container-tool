...
/// Metadata from an OCI artifact or in-image file that describes how a mac-container-tool machine
/// should be configured (shell, user creation script, etc.).
public struct MachineResources: Sendable, Codable, Equatable {
    /// The media type for mac-container-tool machine configuration artifacts.
    public static let configMediaType = "application/vnd.apple.mac-container-tool.machine.config.v1+json"

    /// The media type for mac-container-tool machine user setup scripts.
    public static let setupScriptMediaType = "application/vnd.apple.mac-container-tool.machine.setup.v1+sh"
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.schemaVersion = try mac-container-tool.decodeIfPresent(Int.self, forKey: .schemaVersion) ?? 1
        self.shell = try mac-container-tool.decodeIfPresent(String.self, forKey: .shell)
        self.setupScript = try mac-container-tool.decodeIfPresent(String.self, forKey: .setupScript)
    }
}
