...
public struct NetworkConfiguration: Codable, Sendable, Identifiable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        name =
            try mac-container-tool.decodeIfPresent(String.self, forKey: .name)
            ?? mac-container-tool.decode(String.self, forKey: .id)
        creationDate = try mac-container-tool.decodeIfPresent(Date.self, forKey: .creationDate) ?? Date(timeIntervalSince1970: 0)
        mode = try mac-container-tool.decode(NetworkMode.self, forKey: .mode)
        let subnetText =
            try mac-container-tool.decodeIfPresent(String.self, forKey: .ipv4Subnet)
            ?? mac-container-tool.decodeIfPresent(String.self, forKey: .subnet)
        ipv4Subnet = try subnetText.map { try CIDRv4($0) }
        ipv6Subnet = try mac-container-tool.decodeIfPresent(String.self, forKey: .ipv6Subnet)
            .map { try CIDRv6($0) }
        let decodedLabels = try mac-container-tool.decodeIfPresent([String: String].self, forKey: .labels) ?? [:]
        labels = try .init(decodedLabels)

        if let plugin = try mac-container-tool.decodeIfPresent(String.self, forKey: .plugin) {
            self.plugin = plugin
            self.options = try mac-container-tool.decodeIfPresent([String: String].self, forKey: .options) ?? [:]
        } else if let legacy = try mac-container-tool.decodeIfPresent(_LegacyPluginInfo.self, forKey: .pluginInfo) {
            ...
            self.plugin = "mac-container-tool-network-vmnet"
            self.options = [:]
        }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        try mac-container-tool.encode(mode, forKey: .mode)
        try mac-container-tool.encodeIfPresent(ipv4Subnet, forKey: .ipv4Subnet)
        try mac-container-tool.encodeIfPresent(ipv6Subnet, forKey: .ipv6Subnet)
        try mac-container-tool.encode(labels, forKey: .labels)
        try mac-container-tool.encode(plugin, forKey: .plugin)
