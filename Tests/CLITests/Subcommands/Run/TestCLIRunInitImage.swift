...
/// mac-container-tool starts.
///
/// See: https://github.com/apple/mac-container-tool/discussions/838
///
...
class TestCLIRunInitImage: CLITest {
    ...
    @Test func testRunWithNonExistentInitImage() throws {
        ...
        #expect(throws: CLIError.self, "expected mac-container-tool run with non-existent init-image to fail") {
    ...
    /// Test that the `--init-image` flag works with `mac-container-tool create` command.
    @Test func testCreateWithNonExistentInitImage() throws {
        let name = getTestName()
        let nonExistentImage = "nonexistent.invalid/init-image:does-not-exist"

        #expect(throws: CLIError.self, "expected mac-container-tool create with non-existent init-image to fail") {
            let (_, _, error, status) = try run(arguments: [
                "create",
                "--rm",
                "--name", name,
    ...
    @Test func testRunWithExplicitDefaultInitImage() throws {
        ...
        // Run mac-container-tool with explicit default init image
        try doLongRun(name: name, args: ["--init-image", initImage])
        defer {
            try? doStop(name: name)
        }

        // Verify mac-container-tool is running and functional
