...
/// Stores the mapping between host user and mac-container-tool machine user.
...
public struct MachineConfiguration: Sendable, Codable {
    public static let mac-container-toolUUIDLength = 6

    public static let defaultDNSDomain = "machine"

    /// Identifier for the mac-container-tool machine.
    public var id: String
    /// Image used to create the mac-container-tool machine.
    public var image: ImageDescription
    /// Platform for the mac-container-tool machine
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.id = try mac-container-tool.decode(String.self, forKey: .id)
        self.image = try mac-container-tool.decode(ImageDescription.self, forKey: .image)
        self.platform = try mac-container-tool.decode(ContainerizationOCI.Platform.self, forKey: .platform)
        // DEPRECATED 0.11.0.0 - `decodeIfPresent` used for down-revision compatibility, remove in 0.13.0.0
        self.userSetup = try mac-container-tool.decodeIfPresent(UserSetup.self, forKey: .userSetup) ?? UserSetup(username: NSUserName(), uid: getuid(), gid: getgid())

        try self.validate()
    }

    private func validate() throws {
        let maxNameLength = LinuxContainer.maxIDLength - Self.mac-container-toolUUIDLength - 1
