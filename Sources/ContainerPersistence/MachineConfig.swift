...
/// Boot-time configuration for a mac-container-tool machine.
///
/// These values can be modified without recreating the mac-container-tool machine.
/// Changes take effect on the next boot. `nil` values mean
/// "use the mac-container-tool runtime default."
public struct MachineConfig: Codable, Sendable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        let cpus = try mac-container-tool.decodeIfPresent(Int.self, forKey: .cpus)
        let memory = try mac-container-tool.decodeIfPresent(MemorySize.self, forKey: .memory)
        let homeMount = try mac-container-tool.decodeIfPresent(HomeMountOption.self, forKey: .homeMount)
