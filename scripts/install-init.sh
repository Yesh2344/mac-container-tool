...
Install the init image for mac-container-tool system.
...
CONTAINERIZATION_VERSION="$(${SWIFT} package show-dependencies --format json | jq -r '.dependencies[] | select(.identity == "mac-container-toolization") | .version')"
if [ "${CONTAINERIZATION_VERSION}" == "unspecified" ] ; then
	CONTAINERIZATION_PATH="$(${SWIFT} package show-dependencies --format json | jq -r '.dependencies[] | select(.identity == "mac-container-toolization") | .path')"
	if [ ! -d "${CONTAINERIZATION_PATH}" ] ; then
		echo "editable mac-container-toolization directory at ${CONTAINERIZATION_PATH} does not exist"
		exit 1
	fi
    ...
    sleep 3
	bin/mac-container-tool --debug system start "${START_ARGS[@]}"
	sleep 3
	bin/mac-container-tool i load -i /tmp/init.tar
	rm /tmp/init.tar
fi
