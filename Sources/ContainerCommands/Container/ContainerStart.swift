...
extension Application {
    public struct ContainerStart: AsyncLoggableCommand {
        ...
        public static let configuration = CommandConfiguration(
            commandName: "start",
            abstract: "Start a mac-container-tool")
        ...
        var mac-container-toolId: String

        public func run() async throws {
            var exitCode: Int32 = 127

            let progressConfig = try ProgressConfig(
                description: "Starting mac-container-tool"
            )
            ...
            let mac-container-tool = try await client.get(id: mac-container-toolId)
            ...
            if mac-container-tool.status == .running {
                if !detach {
                    throw ContainerizationError(
                        .invalidArgument,
                        message: "attach is currently unsupported on already running mac-container-tools"
                    )
                }
                print(mac-container-toolId)
                return
            }

            for mount in mac-container-tool.configuration.mounts where mount.isVirtiofs {
            ...
            do {
                let io = try ProcessIO.create(
                    tty: mac-container-tool.configuration.initProcess.terminal,
                ...
                let process = try await client.bootstrap(id: mac-container-tool.id, stdio: io.stdio, dynamicEnv: env)
                progress.finish()

                if detach {
                    try await process.start()
                    try io.closeAfterStart()
                    print(self.mac-container-toolId)
                    return
                }

                exitCode = try await io.handleProcess(process: process, log: log)
            } catch {
                try? await client.stop(id: mac-container-tool.id)

                if error is ContainerizationError {
                    throw error
                }
                throw ContainerizationError(.internalError, message: "failed to start mac-container-tool: \(error)")
            }
