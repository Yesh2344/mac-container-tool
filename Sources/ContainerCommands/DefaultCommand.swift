    ...
    func run() async throws {
        ...
        if pluginLoader == nil {
            throw ValidationError(
                """
                Plugins are unavailable. Start the mac-container-tool system services and retry:
        ...
        guard let plugin = pluginLoader?.findPlugin(name: command), plugin.config.isCLI else {
            throw ValidationError(
                """
                Plugin 'mac-container-tool-\(command)' not found.

                - If system services are not running, start them with: mac-container-tool system start
